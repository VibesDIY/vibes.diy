...
- **Margin beyond the noise band.** Keep an iteration only when the metric gain exceeds the `noiseBand` (the run-to-run jitter measured at the same prompt corpus). A gain inside the band is noise — discard.
