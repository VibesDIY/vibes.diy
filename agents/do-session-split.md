...
4. **The GC: `deleted_classes` migration retiring the old session classes.** A distinct, later step with its own deploy runbook — once drained, a `vN deleted_classes = ["ChatSessions","AppSessions","SharedSessions"]` migration retires them (precedent: `v6` deleted `DocNotify`, `v7` deleted `AccessFnDO`). **Sequence prod-before-cli per [`do-migrations.md`](do-migrations.md):** unlike DocNotify/AccessFnDO (local classes, no cross-script binding — trivial to delete), `APP_SESSIONS` (and `USER_NOTIFY`) are cross-script bound CLI→prod, so deleting them carries the `10061` deploy-ordering trap. Do not fold this into the same deploy as step 2/3 — drain first, then GC in its own migration.
