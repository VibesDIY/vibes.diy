...
### Don't freeze a reactive dependency in a `Lazy` singleton — [#2348](https://github.com/VibesDIY/vibes.diy/issues/2348)

`VibesDiySrvSandbox` is a module-level `Lazy(...)` — it constructs the sandbox once, on whatever route first renders the provider, and `Lazy` returns that first instance forever (later args are ignored). The sandbox captured `vibeApi` from its **constructor** args, so a sandbox first built on a non-vibe render froze `vibeApi === undefined` and answered every vibe-data op with `vibeApi unavailable` — and a sandbox built for vibe A kept routing to A's `vibeApi` after navigating to vibe B. The provider rebuilt the args object each render, which **looked** like it refreshed the sandbox, but `Lazy` discarded the new args; the only symptom that escaped was intermittent console noise on cold loads, masking a real always-wrong-session / wrong-vibe routing landmine. Fix shape: resolve the reactive dependency **live**, not from frozen constructor state — a `setVibeApi()` that updates a stored ref and rewires the `onDocChanged` forwarder (dropping the prior subscription), called by the provider every render and idempotent on an unchanged reference. General rule: when a `Lazy`/`ResolveOnce`/singleton holds something that changes over the page's life (route-derived APIs, auth, current entity), give it a setter that re-binds subscriptions — never read the value off frozen `args`. This is also why the "smallest change, swallow the error" option was wrong: it would have hidden the noise while leaving the wrong-vibe routing in place.
