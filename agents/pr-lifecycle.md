...
5. **Once every feedback comment is resolved, CI is green, and you've validated the change** (or stated in one line why nothing was reachable — see [Validate changed features](#validate-changed-features-against-the-preview--standard-operating-procedure), which is SOP), **label the PR `ready-to-merge`** and pair it with the Rollout watch comment. See [Ready-to-merge signal](#ready-to-merge-signal) below.
6. **Then merge it — `ready-to-merge` is a waypoint, not the finish line.** For **garden-variety** changes (pure library code, docs, tests, blog seeds, off-by-default flag _plumbing_, small non-risky fixes — anything whose rollback is a clean `git revert` with no deploy coupling), **merge it yourself with `rebase`** the moment Charlie's feedback is resolved and CI is green, then continue to the next thing. **Hold for a human merge only** when the change is risky per the [Autonomous merge loop](#autonomous-merge-loop--and-when-to-hold-for-a-human) exception list (schema/migration, Durable Object topology, new bindings/infra, queue behavior, live env-flag flips, or any non-clean-revert) — for those, stop at the label + Rollout watch and say in the note that it's a hold-for-human deploy and why. **When unsure, hold.** And do **not** schedule a twenty-minute check-in as a stand-in for a merge you're already cleared to do — if it's garden-variety and green, merge now; a check-in is only for work that is genuinely still pending _and_ webhook-silent.
...
**The label is not the finish line.** For garden-variety changes it's immediately followed by **your own rebase-merge** (step 6 above / [Autonomous merge loop](#autonomous-merge-loop--and-when-to-hold-for-a-human)). It's a true hand-off to a human only for the risky bucket that holds for a human deploy — and in that case the Rollout watch note must say so explicitly.
