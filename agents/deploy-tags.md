...
- **`checks`** — lint + format-check + build(typecheck) (`run-tests: 'false'` on
  `actions/base`); **skipped on an already-green SHA**. Every workflow that emits
  a `compile_test` check now runs the real suite (`ci.yaml`, this deploy,
  `call-ai-publish`) — the test-skipping publish jobs were renamed off that check
  name in #2790 — so a green `compile_test` certifies lint+build+test all passed
  and re-running lint/build here would be waste.
...
- **`compile_test`** — aggregator that **keeps this exact job name** so it still
  emits a reusable green `compile_test` check on the deploy SHA. Green iff `gate`
  succeeded **and** the SHA was already green **or** (`checks` + every `test`
  shard passed).
...
> **Build tool: `@fireproof/core-cli`.** Each package's `pack`/`publish` script is `core-cli build [--doPack]` — an isolated publish build (copies only that package's sources into `dist/npm/`, runs a standalone `tsc` with `include: ["**/*"]`, emits the export map, packs/publishes). That isolation is what the required `publish_build` CI guard exercises (`pnpm -r run --if-present pack`). This build-time coupling to `core-cli` is **Bucket F** (#2483); the decision is to **keep it for now** — inventory, options, rationale, and the revisit-triggers are in [`docs/superpowers/specs/2026-06-30-bucket-f-core-cli-build-tool-design.md`](../docs/superpowers/specs/2026-06-30-bucket-f-core-cli-build-tool-design.md). Don't replace the publish build casually: it ships npm bytes and esm.sh caches bad URLs.
