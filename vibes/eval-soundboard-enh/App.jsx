...
function Recorder({ viewer }) {
  ...
  async function submit() {
    ...
    try {
      ...
      let category = "misc"
      try {
        const ai = await callAI(`Suggest a short label (max 4 words) and one category (intro, sfx, music, voice, misc) for this audio clip described as: "${desc}"`, {
        ...
        {!blob ? (
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              >
                {busy && <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeDasharray="50" /></svg>}
                {busy ? "Submitting..." : "Submit for review"}
              </button>
              <button onClick={() => { setBlob(null); setDesc("") }} className="min-h-[44px] px-4 rounded-lg border border-[#d1d4dc]">Discard</button>
            </div>
          </>
        )}
      </div>
          ...
          {pending.map((d) => (
            <li key={d._id} className="border border-[#e5e7eb] rounded-lg p-3 bg-[#fafbfc]">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  {["intro", "sfx", "music", "voice", "misc"].map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="min-h-[40px] px-3 rounded border border-[#d1d4dc] text-sm"
                >Reject</button>
              </div>
            </li>
          ))}
