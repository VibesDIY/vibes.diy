...
// ── Contention config (this app: calm worker) ───────────────────────
const CONFIG = {
  title: "Single Snap",
  slots: 1, // one image, nothing else in flight
  promptStyle: "short", // fast generation → narrow put/get window
  noiseWriter: false,
  noiseEveryMs: 70,
  companionWrites: false, // no companion docs → no cross-db contention
};

const SHORT = ["a red apple", "a blue cup", "a yellow star", "a green leaf", "a white cloud", "a purple grape"];
...
function Slot({ runId, idx, onDone, result }) {
  ...
  const base = CONFIG.promptStyle === "short" ? SHORT[idx % SHORT.length] : LONG[idx % LONG.length];
  const prompt = `${base} — run ${runId}`;
  ...
  useEffect(() => {
    if (CONFIG.companionWrites && !companionRef.current) {
      ...
      parentDb
        .put({ _id: `parent-${runId}-${idx}`, type: "parent", payload: "x".repeat(3000), created: Date.now() })
        .catch(() => {});
    }
  }, [parentDb]);
