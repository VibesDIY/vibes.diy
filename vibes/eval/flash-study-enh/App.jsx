...
function DeckCreator({ c, viewer, database, onCreated }) {
  ...
  async function suggest() {
    ...
    try {
      const res = await callAI(`Generate 5 flashcards (question + answer) for topic: ${topic}`, {
        schema: { properties: { cards: { type: "array", items: { type: "object", properties: {
  ...
  function removeDraft(i) { setDrafts(drafts.filter((_, idx) => idx !== i)) }
    ...
    <section id="deck-creator" className={`${c.card} ${c.border} border rounded-2xl p-4`}>
      ...
      <form onSubmit={createDeck} className="space-y-3">
        ...
        <div className="flex gap-2">
          ...
          />
          <button type="button" onClick={suggest} disabled={isLoading || !topic.trim()}
          ...
          <ul className="space-y-2">
            {drafts.map((d, i) => (
              <li key={i} className={`border ${c.border} rounded-xl p-2 space-y-1`}>
                <input value={d.question} onChange={e => updateDraft(i, "question", e.target.value)}
                  className="w-full text-sm font-medium bg-transparent outline-none" placeholder="Question" />
                <input value={d.answer} onChange={e => updateDraft(i, "answer", e.target.value)}
                  className={`w-full text-sm bg-transparent outline-none ${c.muted}`} placeholder="Answer" />
                <button type="button" onClick={() => removeDraft(i)} className={`${c.muted} text-xs`}>remove</button>
              </li>
            ))}
          </ul>
        )}
...
function StudyMode({ c, viewer, deck, cards }) {
  ...
  React.useEffect(() => { setIdx(0); setFlipped(false) }, [deck?._id])
      ...
      {cards.length === 0 ? (
        ...
        <>
          <button onClick={() => setFlipped(f => !f)}
            ...
            <div className={`${c.muted} text-xs uppercase tracking-wide mb-2`}>{flipped ? "Answer" : "Question"} · {idx + 1}/{cards.length}</div>
          ...
          {flipped ? (
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => record("wrong")} className="bg-[#fde8ea] text-[#e63946] rounded-xl py-3 font-medium min-h-[44px]">Got it wrong</button>
              <button onClick={() => record("right")} className="bg-green-100 text-green-700 rounded-xl py-3 font-medium min-h-[44px]">Got it right</button>
            </div>
          ) : (
            <div className="flex gap-2">
              <button onClick={() => { setIdx(i => (i - 1 + cards.length) % cards.length); setFlipped(false) }}
                className={`flex-1 border ${c.border} rounded-xl py-3 min-h-[44px]`}>Prev</button>
              <button onClick={() => { setIdx(i => (i + 1) % cards.length); setFlipped(false) }}
                className={`flex-1 border ${c.border} rounded-xl py-3 min-h-[44px]`}>Next</button>
            </div>
          )}
        </>
      )}
...
export default function App() {
  ...
  return (
    <main id="app" className={`${c.page} min-h-screen pb-24`}>
      <header id="app-header" className={`${c.card} ${c.border} border-b sticky top-0 z-10 px-4 py-3 flex items-center justify-between`}>
        <div>
