...
function Stage({ question }) {
  return (
    <section id="stage" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(1_0_0)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
...
function TopicPicker({ isOwner, database }) {
  ...
  if (!isOwner) {
    return (
      <section id="topic" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(0.85_0.18_85)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
  ...
  async function suggest() {
    setSuggesting(true)
    try {
      const r = await callAI("Suggest one fun trivia topic in 2-4 words.", { schema: { properties: { topic: { type: "string" } } } })
  ...
  const Spinner = () => <svg className="animate-spin inline w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>

  return (
    <section id="topic" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(0.85_0.18_85)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
      <h2 className="font-bold text-lg mb-2 tracking-wide uppercase">Host Controls</h2>
      <div className="flex gap-2 mb-2">
        <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. 80s movies" className="flex-1 border-2 border-[oklch(0.15_0.02_280)] rounded px-2 py-2 bg-white min-h-[44px]" />
        <button onClick={suggest} disabled={suggesting} className="border-2 border-[oklch(0.15_0.02_280)] bg-white px-3 rounded font-bold text-sm min-h-[44px]">{suggesting ? <Spinner/> : "✦"}</button>
      </div>
      <button onClick={generate} disabled={loading || !topic.trim()} className="w-full border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(0.55_0.24_28)] text-white py-2 rounded font-bold uppercase tracking-wide min-h-[44px] disabled:opacity-50">
...
function AnswerBox({ viewer, question, database, answers }) {
  ...
  const [loading, setLoading] = React.useState(false)
  const alreadyAnswered = viewer && question && answers.some(a => a.questionId === question._id && a.authorHandle === viewer.userHandle)

  if (!viewer) {
    return (
      <section id="answer" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(1_0_0)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
    ...
    <section id="answer" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(1_0_0)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
      ...
      {alreadyAnswered ? (
        ...
        <form onSubmit={submit} className="space-y-2">
          <input value={text} onChange={e => setText(e.target.value)} placeholder="Type your answer…" className="w-full border-2 border-[oklch(0.15_0.02_280)] rounded px-2 py-2 min-h-[44px]" />
          <button type="submit" disabled={loading || !text.trim()} className="w-full border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(0.52_0.18_255)] text-white py-2 rounded font-bold uppercase tracking-wide min-h-[44px] disabled:opacity-50">
            {loading ? <><Spinner/> Judging…</> : "Submit"}
          </button>
        </form>
      )}
    </section>
...
function Scoreboard({ answers }) {
  ...
  const totals = {}
  answers.forEach(a => { totals[a.authorHandle] = (totals[a.authorHandle] || 0) + (a.points || 0) })
  const sorted = Object.entries(totals).sort((a,b) => b[1] - a[1])

  return (
    <section id="scoreboard" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(0.62_0.19_145)] text-white rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
      <h2 className="font-bold text-lg mb-2 tracking-wide uppercase">Scoreboard</h2>
      {sorted.length === 0 ? <p className="text-sm">No scores yet.</p> : (
        <ul className="space-y-2">
          {sorted.map(([handle, pts], i) => (
            <li key={handle} className="flex items-center justify-between bg-white text-[oklch(0.15_0.02_280)] rounded px-3 py-2 border-2 border-[oklch(0.15_0.02_280)]">
              <span className="flex items-center gap-2"><span className="font-black">#{i+1}</span><ViewerTag userHandle={handle} /></span>
...
function History({ questions, answers }) {
  ...
  return (
    <section id="history" className="border-2 border-[oklch(0.15_0.02_280)] bg-[oklch(1_0_0)] rounded p-4 shadow-[4px_4px_0_oklch(0.15_0.02_280)]">
      <h2 className="font-bold text-lg mb-2 tracking-wide uppercase">Round History</h2>
      {sortedQ.length === 0 ? <p className="text-sm text-[oklch(0.50_0.02_280)]">Past questions will appear here.</p> : (
...
export default function App() {
  ...
  return (
    <main id="app" className="min-h-screen bg-[oklch(0.96_0.01_90)] text-[oklch(0.15_0.02_280)] font-[Space_Grotesk,sans-serif]">
      <header id="app-header" className="border-b-4 border-[oklch(0.15_0.02_280)] bg-[oklch(0.55_0.24_28)] text-white px-4 py-3 flex items-center justify-between sticky top-0 z-10">
