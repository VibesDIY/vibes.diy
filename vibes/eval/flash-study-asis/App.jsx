...
function GenerateDeck({ database, isOwner, viewer }) {
  ...
  async function generate() {
    ...
    try {
      ...
      const deckRes = await database.put({ type: "deck", topic: topic.trim(), createdAt: Date.now(), order: cards.map((_, i) => i) })
...
function CardViewer({ database, deck, cards, isOwner, viewer, progress }) {
  ...
  async function toggleLearned() {
    ...
    if (existing) {
      ...
      await database.put({ type: "progress", cardId: current._id, deckId: deck._id, userHandle: viewer.userHandle, createdAt: Date.now() })
    }
  }
  ...
  return (
    <section id="cards" className="bg-[oklch(0.13_0_0)] text-[oklch(1_0_0)] rounded-[32px] p-5 border border-[oklch(1_0_0/0.20)]">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-[oklch(1_0_0/0.50)]">{idx + 1} / {orderedCards.length}</span>
      </div>
      {current ? (
        <>
          <button
          ...
          <div className="flex gap-2 mb-2">
            ...
            <button
              onClick={() => { setIdx((i) => Math.min(orderedCards.length - 1, i + 1)); setFlipped(false) }}
