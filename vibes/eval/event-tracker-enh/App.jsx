...
function DayChips({ c }) {
  return (
    <section id="day-chips" className={`${c.chipsBg} px-3 py-3 flex gap-2 overflow-x-auto sticky top-[60px] z-10 border-b ${c.headerBorder}`}>
...
export default function App() {
  ...
  return (
    <div className={c.page}>
      ...
      <section id="day-chips" className={`${c.chipsBg} px-3 py-3 flex gap-2 overflow-x-auto sticky top-[60px] z-10 border-b ${c.headerBorder}`}>
        <button onClick={() => setActiveDay("all")} className={`${activeDay === "all" ? c.chipActive : c.chipIdle} px-4 py-2 rounded-full text-sm whitespace-nowrap min-h-[44px]`}>All Days</button>
        {days.map(d => (
          <button key={d} onClick={() => setActiveDay(d)} className={`${activeDay === d ? c.chipActive : c.chipIdle} px-4 py-2 rounded-full text-sm whitespace-nowrap min-h-[44px]`}>{d}</button>
        ))}
      </section>
      <main id="app">
            ...
            <article key={act._id} className={`${c.cardBg} ${c.cardBorder} border-2 rounded-xl p-4`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#00f0ff] text-sm">{act.stage} · {act.day} {act.time}</p>
                ...
                <div className="flex flex-col gap-2 items-end">
                  <button onClick={() => toggleStar(act._id)} disabled={!viewer} className={`${starredIds.has(act._id) ? c.starOn : c.starOff} min-h-[44px] min-w-[44px] flex items-center justify-center text-3xl disabled:opacity-40`} aria-label="Star act">
                    <svg viewBox="0 0 24 24" width="28" height="28" fill={starredIds.has(act._id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15 9 22 9.5 17 14.5 18.5 22 12 18 5.5 22 7 14.5 2 9.5 9 9 12 2"/></svg>
                  </button>
                  {isCrew && (
                    <button onClick={() => deleteAct(act._id)} className="text-[#f93c94] text-xs underline">delete</button>
                  )}
                </div>
              </div>
            </article>
