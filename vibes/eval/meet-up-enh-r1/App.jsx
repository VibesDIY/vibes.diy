...
function OrganizerView() {
  ...
  const suggest = async () => {
    ...
    try {
      ...
      const res = await callAI(`Given these availability windows from multiple people, pick the best overlapping meeting time and explain briefly:\n\n${summary}`, {
        schema: { properties: { recommendation: { type: "string" }, rationale: { type: "string" } } },
      })
      const parsed = JSON.parse(res)
      await database.put({ type: "suggestion", recommendation: parsed.recommendation, rationale: parsed.rationale, createdAt: Date.now() })
    } finally {
      setLoading(false)
    }
  }
  ...
  const c = {
    ...
    heading: "text-[#D4FF00] font-semibold mb-3 text-lg",
    hint: "text-[#888888] text-sm",
    button: "bg-[#D4FF00] text-black font-semibold px-4 py-3 rounded-md min-h-[44px] w-full disabled:opacity-50 flex items-center justify-center gap-2",
    ...
    <section id="organizer-view" className={c.section}>
      ...
      {Object.entries(byPerson).map(([handle, slots]) => (
        <div key={handle} className={c.person}>
          ...
          {slots.map((s) => (
            <div key={s._id} className={c.slot}>{s.label}: {s.start} → {s.end}</div>
          ))}
        </div>
      ))}
      <button className={c.button + " mt-3"} onClick={suggest} disabled={loading || docs.length === 0}>
        {loading && <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>}
