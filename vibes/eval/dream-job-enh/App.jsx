...
function CaptureSection({ viewer, database }) {
  ...
  const c = {
    ...
    input: "w-full px-3 py-3 min-h-[44px] border-2 border-[oklch(0.12_0.01_0)] rounded bg-[oklch(0.93_0.12_95)] text-[oklch(0.12_0.01_0)] font-medium",
    ...
    preview: "w-full aspect-square bg-[oklch(0.93_0.03_130)] border-2 border-dashed border-[oklch(0.12_0.01_0)] rounded flex items-center justify-center text-[oklch(0.45_0.01_0)]",
  ...
  return (
    <section id="capture" className={c.card}>
      <h2 className={c.heading} style={{ fontFamily: "Caveat, cursive" }}>Your turn!</h2>
      <form onSubmit={submit} className="space-y-3">
        <div>
          ...
          />
          {selfiePreview && (
            <img src={selfiePreview} alt="selfie preview" className="mt-2 w-32 h-32 object-cover border-2 border-[oklch(0.12_0.01_0)] rounded" />
          )}
        </div>
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={suggestJob} disabled={isLoading} className={c.suggest}>
              {isLoading ? (
                <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeDasharray="40 20" /></svg>
              ) : "Surprise me"}
            </button>
          </div>
        </div>
      ...
      {generated && (
        <div className="mt-4 p-3 bg-[oklch(0.90_0.05_240)] border-2 border-[oklch(0.12_0.01_0)] rounded">
          <p className="text-sm font-semibold">Generating: <em>{generated.caption}</em></p>
        </div>
      )}
    </section>
  )
  ...
  return (
    <article className={c.tile} style={{ transform: `rotate(${tilt}deg)` }}>
      <div className="relative">
        <ImgGen prompt={doc.imagePrompt} database="dreamJobs" className="w-full aspect-square object-cover border-2 border-[oklch(0.12_0.01_0)] rounded bg-[oklch(0.93_0.03_130)]" showControls={false} />
        {selfie?.url && (
          <img src={selfie.url} alt="selfie" className="absolute bottom-2 right-2 w-12 h-12 object-cover border-2 border-[oklch(0.12_0.01_0)] rounded shadow-[2px_2px_0_oklch(0.12_0.01_0)]" />
        )}
      </div>
      <p className={c.job} style={{ fontFamily: "Caveat, cursive" }}>{doc.dreamJob}</p>
...
function WallSection({ database, ViewerTag }) {
  ...
  return (
    <section id="wall">
      <h2 className={c.heading} style={{ fontFamily: "Caveat, cursive" }}>The Party Wall ({portraits.length})</h2>
      {portraits.length === 0 ? (
        <p className={c.empty}>No portraits yet. Be the first!</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {portraits.map(d => <PortraitTile key={d._id} doc={d} ViewerTag={ViewerTag} />)}
        </div>
      )}
    </section>
  )
...
export default function App() {
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 bg-[oklch(0.93_0.12_95)] border-b-4 border-[oklch(0.12_0.01_0)] px-4 py-3 flex items-center justify-between",
  ...
  return (
    <div className={c.page} style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <header id="app-header" className={c.header}>
        <h1 className={c.title} style={{ fontFamily: "Caveat, cursive" }}>Dream Job Wall</h1>
        <ViewerTag />
      </header>
