    ...
    <div className={`${c.panelHi} border ${c.border} rounded-sm p-4 flex flex-col gap-3`}>
      ...
      {isOwner && (
        <div className="grid grid-cols-4 gap-2">
          {!timer.running ? (
            <button onClick={start} className={`${c.accent} text-xs tracking-widest py-2 rounded-sm min-h-[44px] col-span-2`}>START</button>
          ) : (
            <button onClick={pause} className={`${c.panel} border ${c.border} text-xs tracking-widest py-2 rounded-sm min-h-[44px] col-span-2`}>PAUSE</button>
          )}
          <button onClick={reset} className={`${c.panel} border ${c.border} text-xs tracking-widest py-2 rounded-sm min-h-[44px]`}>RESET</button>
          <button onClick={del} className={`${c.panel} border ${c.border} text-xs tracking-widest py-2 rounded-sm min-h-[44px] text-[oklch(0.53_0.22_25)]`}>DEL</button>
        </div>
      )}
    </div>
  )
    ...
    <section id="timer-grid" className={`${c.panel} border ${c.border} rounded-sm p-4`}>
      ...
      {timers.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {timers.map((t) => <TimerCard key={t._id} timer={t} c={c} database={database} isOwner={isOwner} />)}
        </div>
      )}
    </section>
  )
...
function CoachPanel({ c, sessions, database, isOwner }) {
  ...
  async function getSuggestion() {
    setLoading(true)
    try {
      const summary = focusSessions.slice(0, 10).map((s) => `${s.timerName}: ${Math.round(s.durationMs / 60000)}min`).join("; ")
  ...
  return (
    <section id="coach-panel" className={`${c.panel} border ${c.border} rounded-sm p-4`}>
      <div className="flex items-center justify-between mb-3">
        ...
        {isOwner && (
          <button onClick={getSuggestion} disabled={loading} className={`${c.accent} text-xs tracking-widest px-3 py-2 rounded-sm min-h-[44px] flex items-center gap-2 disabled:opacity-50`}>
            {loading && <svg className="animate-spin w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>}
      ...
      {latestCoach ? (
        <div className="space-y-2">
          ...
          <p className={`${c.muted} text-xs`}>Next focus: <span className={c.brand}>{latestCoach.nextFocusMin} min</span></p>
...
export default function App() {
  ...
  const c = {
    ...
    input: "bg-[oklch(0.10_0.003_264)] border-[oklch(0.24_0.003_264)] text-[oklch(0.93_0.005_264)] placeholder:text-[oklch(0.63_0.008_264)]",
  }
