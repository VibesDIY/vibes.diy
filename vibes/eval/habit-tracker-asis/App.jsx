...
function Header({ ViewerTag, c }) {
  return (
    <header id="app-header" className={`sticky top-0 z-10 ${c.headerBg} ${c.border} border-b backdrop-blur-md px-4 py-3 flex items-center justify-between`}>
      <div className="flex items-center gap-2">
        <svg viewBox="0 0 24 24" className={`w-7 h-7 ${c.flame}`} fill="currentColor"><path d="M12 2s4 5 4 9a4 4 0 01-8 0c0-1 .5-2 1-3-2 1-4 4-4 7a7 7 0 0014 0c0-6-7-13-7-13z"/></svg>
        <h1 className={`font-bold text-xl ${c.ink}`} style={{fontFamily:'Fredoka, sans-serif'}}>Hearth Streaks</h1>
      </div>
      <ViewerTag />
    </header>
  )
  ...
  return (
    <section id="coach" className={`mx-4 mt-4 ${c.coachBg} ${c.border} border rounded-2xl p-4`}>
      <div className="flex items-start gap-3">
        <svg viewBox="0 0 24 24" className={`w-6 h-6 ${c.gold} shrink-0 mt-0.5`} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a7 7 0 00-7 7c0 3 2 5 3 6v3a2 2 0 002 2h4a2 2 0 002-2v-3c1-1 3-3 3-6a7 7 0 00-7-7z"/></svg>
        {nudge ? (
  ...
  return (
    <li className={`${c.cardBg} ${c.border} border rounded-2xl p-4 flex items-center gap-3`}>
      <div className="flex-1 min-w-0">
        <div className={`font-semibold ${c.ink} truncate`} style={{fontFamily:'Fredoka, sans-serif'}}>{habit.name}</div>
        <div className="flex items-center gap-1.5 mt-1">
          <svg viewBox="0 0 24 24" className={`w-4 h-4 ${streak > 0 ? c.flame : 'text-white/30'}`} fill="currentColor"><path d="M12 2s4 5 4 9a4 4 0 01-8 0c0-1 .5-2 1-3-2 1-4 4-4 7a7 7 0 0014 0c0-6-7-13-7-13z"/></svg>
          <span className={`text-sm ${c.ink} opacity-80`}>{streak} day{streak === 1 ? '' : 's'}</span>
        </div>
      </div>
      {isOwner && (
        <button
...
function HabitList({ habits, checkinsByHabit, onCheckIn, isOwner, c }) {
  return (
    <section id="habits" className="px-4 pt-4 pb-32 space-y-3">
      ...
      {habits.length === 0 ? (
        <div className={`${c.cardBg} ${c.border} border rounded-2xl p-6 text-center ${c.ink} opacity-70`}>
          No habits yet.
        </div>
      ) : (
        <ul className="space-y-3">
          {habits.map(h => (
            <HabitRow key={h._id} habit={h} checkinDays={checkinsByHabit[h._id] || []} onCheckIn={onCheckIn} isOwner={isOwner} c={c} />
          ))}
        </ul>
      )}
    </section>
...
function AddHabitBar({ database, c }) {
  ...
  return (
    <section id="add-habit" className={`fixed bottom-0 left-0 right-0 ${c.barBg} ${c.border} border-t px-4 py-3 backdrop-blur-md`}>
      {!open ? (
        <button onClick={() => setOpen(true)} className={`w-full min-h-[52px] ${c.primary} ${c.ink} font-semibold rounded-2xl flex items-center justify-center gap-2 shadow-lg`}>
          <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14M5 12h14"/></svg>
          Add habit
        </button>
      ) : (
        <form onSubmit={add} className="flex gap-2 items-center">
          ...
          />
          <button type="button" onClick={suggest} disabled={loading} title="Suggest" className={`min-h-[48px] min-w-[48px] rounded-xl bg-white/10 ${c.ink} flex items-center justify-center`}>
            {loading ? (
              <svg viewBox="0 0 24 24" className="w-5 h-5 animate-spin" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>
            ) : (
              <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l2 5 5 2-5 2-2 5-2-5-5-2 5-2z"/></svg>
            )}
          </button>
          <button type="submit" className={`min-h-[48px] px-4 ${c.primary} ${c.ink} font-semibold rounded-xl`}>Add</button>
          <button type="button" onClick={() => { setOpen(false); setName("") }} className={`min-h-[48px] min-w-[48px] ${c.ink} opacity-60`}>✕</button>
        </form>
      )}
    </section>
  )
  ...
  React.useEffect(() => {
    ...
    for (const habit of activeHabits) {
      ...
      if (streak === 0 && daysSince >= 2 && daysSince <= 30 && !coachedRef.current.has(habit._id)) {
        ...
        callAI(`A user's habit "${habit.name}" lost its streak. They missed ${daysSince} days. Write a warm, brief encouragement (1-2 sentences) and a tiny micro-goal to restart.`, {
          schema: { properties: { message: { type: "string" }, microGoal: { type: "string" } } }
        }).then(res => {
          const { message, microGoal } = JSON.parse(res)
          database.put({ type: "nudge", habitId: habit._id, habitName: habit.name, message, microGoal, daysMissed: daysSince, createdAt: Date.now() })
        }).catch(() => {})
      }
    }
