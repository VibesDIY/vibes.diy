...
function Pad({ index, padDoc, kit, isOwner, onUpload, onRename }) {
  ...
  const trigger = async () => {
    ...
    if (padDoc?._files?.sample) {
      try { const f = await padDoc._files.sample.file(); await playBuffer(f) } catch {}
    } else {
    ...
    <div className="relative aspect-square">
      ...
      {isOwner && (
        <div className="absolute -bottom-1 left-0 right-0 flex justify-center gap-1 opacity-0 hover:opacity-100 focus-within:opacity-100">
          ...
          >load</button>
          <button
            onClick={() => { const n = prompt("Pad name?", label); if (n) onRename(index, n) }}
...
export default function App() {
  ...
  const padByIndex = {}
  pads.forEach(p => { padByIndex[p.padIndex] = p })
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 bg-[#2a0a2e]/95 backdrop-blur border-b-2 border-[#f93c94] px-4 py-3 flex items-center justify-between",
            ...
            <section id="kit-switcher" aria-label="Kit switcher" className={c.kitBar}>
              {Object.entries(KITS).map(([key, k]) => (
                <button
                  key={key}
                  onClick={() => setKit(key)}
                  className={`${c.kitBtn} ${kit === key ? c.kitActive : c.kitIdle}`}
                >{k.name}</button>
              ))}
            </section>
          )}
          {!isOwner && (
            <p className={`${c.kitBar} text-[#2a0a2e] [font-family:Orbitron,sans-serif] text-sm`}>
              kit: {KITS[kit].name}
            </p>
          )}
          ...
          {isOwner && (
            <section id="clear-controls" className="mt-4 flex flex-wrap gap-1 justify-center">
              {pads.map(p => (
                ...
                >clear {p.label || `pad ${p.padIndex + 1}`}</button>
              ))}
            </section>
          )}
