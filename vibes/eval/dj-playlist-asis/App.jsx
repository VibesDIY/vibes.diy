  ...
  return (
    <article className="rounded-2xl bg-[#2a0a2e]/95 border-2 border-[#00f0ff] overflow-hidden shadow-[0_0_16px_rgba(0,240,255,0.35)]">
      <button onClick={() => setOpen(!open)} className="w-full text-left p-4 min-h-[44px]">
        ...
        <p className="font-['Share_Tech_Mono'] text-[#fcee0a]/80 text-xs mt-2">{tape.tracks?.length || 0} tracks · tap to {open ? "close" : "open"}</p>
      </button>
      {open && (
          ...
          {tape.tracks?.map((t, i) => (
            <li key={i} className="p-3 bg-[#4d1558]/40">
              <div className="flex items-baseline justify-between gap-3">
                ...
                <a href={t.youtubeUrl} target="_blank" rel="noopener noreferrer" className="shrink-0 min-h-[36px] px-3 inline-flex items-center rounded-md bg-[#f93c94] text-[#ffffff] font-['Orbitron'] text-xs tracking-wider">▶ YT</a>
              </div>
