...
function ComposeRecipe({ viewer, database }) {
  ...
  async function suggest() {
    setSuggesting(true)
    try {
      const res = await callAI("Invent one absurd recipe. Return a short whimsical title and an ingredient string made of 5-8 food emoji only (no text).", {
  ...
  async function submit(e) {
    ...
    await database.put({
      type: "recipe", title: title.trim(), emojis: emojis.trim(),
      authorHandle: viewer.userHandle, createdAt: Date.now()
    })
    setTitle(""); setEmojis("")
  }

  const inp = "w-full bg-[oklch(0.17_0.000_0)] border border-[oklch(0.37_0.03_260)] rounded px-3 py-3 min-h-[44px] text-[oklch(0.93_0.006_265)] focus:outline-none focus:border-[oklch(0.71_0.02_261)]"
  const btn = "min-h-[44px] px-4 py-2 rounded border border-[oklch(0.37_0.03_260)] bg-[oklch(0.25_0.01_260)] hover:bg-[oklch(0.30_0.01_260)] disabled:opacity-50"

  return (
    <section id="compose" className="border border-[oklch(0.37_0.03_260)] rounded-lg p-4 bg-[oklch(0.20_0.005_260)]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
      ...
      {viewer && (
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex gap-2">
            <button type="submit" className={btn + " flex-1"}>Publish</button>
            <button type="button" onClick={suggest} disabled={suggesting} className={btn} title="AI suggestion">
              {suggesting ? (
                <svg className="animate-spin w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>
              ) : "✨ Inspire"}
            </button>
          </div>
        </form>
...
function RecipeFeed({ recipes, roasts, viewer, database, ViewerTag }) {
  ...
  async function roast(recipe) {
    ...
    try {
      const res = await callAI(`You are an acerbic, witty food critic. Roast this recipe brutally but cleverly. Title: "${recipe.title}". Ingredients: ${recipe.emojis}. Return a snarky verdict paragraph, scores 0-10 for creativity, plausibility, and deliciousness, and an overall rating out of 10.`, {
        schema: { properties: {
          verdict: { type: "string" },
              ...
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                {roastDoc && (
                  <div className="text-right">
                    <div className="text-3xl font-bold">{roastDoc.overall}<span className="text-base text-[oklch(0.71_0.02_261)]">/10</span></div>
                  </div>
                )}
              </div>
                ...
                <button onClick={() => roast(r)} disabled={roastingId === r._id} className={btn + " mt-2"}>
                  {roastingId === r._id ? (
                    <span className="flex items-center gap-2">
                      <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>
                      Critic tasting…
                    </span>
                  ) : "🔥 Roast This"}
                </button>
