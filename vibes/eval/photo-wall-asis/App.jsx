  ...
  return (
    <div className="mb-4 break-inside-avoid rounded-lg overflow-hidden border border-[oklch(0.28_0.03_257)] bg-[oklch(0.16_0.000_0)]">
      {doc._files?.photo?.url && (
        <img src={doc._files.photo.url} alt={doc.caption || "uploaded image"} className="w-full block" />
      )}
      <div className="p-3">
        ...
        {!doc.analyzed ? (
          <p className="text-xs text-[oklch(0.71_0.02_261)] inline-flex items-center gap-2">
            <svg className="animate-spin" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>
            Analyzing…
          </p>
        ) : editing ? (
          <div className="space-y-2 animate-[fadeIn_0.3s_ease-in]">
            ...
            <div className="flex gap-2">
              <button onClick={save} className="text-xs px-3 py-1.5 rounded bg-[oklch(1.00_0.000_0)] text-[oklch(0.14_0.000_0)] font-medium min-h-[32px]">Save</button>
              <button onClick={() => setEditing(false)} className="text-xs px-3 py-1.5 rounded bg-[oklch(1.00_0.000_0_/_0.1)] text-[oklch(1.00_0.000_0)] min-h-[32px]">Cancel</button>
            </div>
          </div>
        ) : (
          <div className="animate-[fadeIn_0.5s_ease-in]">
            {doc.tags?.length > 0 && (
              <ul className="flex flex-wrap gap-1.5 mb-2">
                {doc.tags.map((tag, i) => (
                  <li key={i} className="text-xs px-2 py-0.5 rounded-full bg-[oklch(1.00_0.000_0_/_0.1)] text-[oklch(1.00_0.000_0)]">{tag}</li>
                ))}
              </ul>
            )}
            {doc.caption && <p className="text-xs text-[oklch(0.71_0.02_261)] leading-relaxed">{doc.caption}</p>}
            {canEdit && (
              <button onClick={() => setEditing(true)} className="text-xs text-[oklch(0.71_0.02_261)] hover:text-[oklch(1.00_0.000_0)] mt-2 underline">Edit</button>
            )}
          </div>
        )}
      </div>
...
export default function App() {
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 bg-[oklch(0.14_0.000_0)]/95 backdrop-blur border-b border-[oklch(0.28_0.03_257)] px-4 py-3 flex items-center justify-between",
