...
function CreateSection({ isOwner, database }) {
  ...
  async function create(e) {
    ...
    await database.put({
      type: "timer", label: label.trim(),
      workMin: Number(workMin) || 25, breakMin: Number(breakMin) || 5,
      phase: "work", running: false, phaseStart: Date.now(),
      pausedElapsed: 0, completedRounds: 0, totalFocusSec: 0,
      createdAt: Date.now(),
    })
    setLabel("")
  }
  const inp = "min-h-[44px] px-3 rounded bg-[oklch(0.20_0.005_264)] border border-[oklch(0.24_0.003_264)] text-[oklch(0.93_0.005_264)] w-full"
  return (
    <section id="create" className="p-4 border-t border-[oklch(0.24_0.003_264)]">
      ...
      <form onSubmit={create} className="space-y-2">
        ...
        <div className="flex gap-2">
          <label className="flex-1 text-xs text-[oklch(0.63_0.008_264)]">Work min
            <input type="number" min="1" className={inp} value={workMin} onChange={(e) => setWorkMin(e.target.value)} />
          </label>
          <label className="flex-1 text-xs text-[oklch(0.63_0.008_264)]">Break min
            <input type="number" min="1" className={inp} value={breakMin} onChange={(e) => setBreakMin(e.target.value)} />
          </label>
        </div>
        <button type="submit" className="min-h-[44px] w-full rounded bg-[oklch(0.53_0.22_25)] text-[oklch(0.93_0.005_264)] uppercase tracking-wider font-semibold">Create Timer</button>
      </form>
    </section>
  )
}
...
function StatsSection({ timers }) {
  ...
  const hrs = Math.floor(totalFocus / 3600), mins = Math.floor((totalFocus % 3600) / 60)
    ...
    <section id="stats" className="p-4 border-t border-[oklch(0.24_0.003_264)]">
      ...
      <div className="flex gap-2">
        ...
        <div className={cell}>
          ...
          <div className="text-2xl font-bold tabular-nums">{hrs}h {mins}m</div>
        </div>
        <div className={cell}>
...
function CoachSection({ tips }) {
  ...
  return (
    <section id="coach" className="p-4 border-t border-[oklch(0.24_0.003_264)] pb-24">
      ...
      {coachTips.length === 0 && <p className="text-sm text-[oklch(0.63_0.008_264)] italic">Tips appear after each work session completes.</p>}
