...
function SurveyForm({ database }) {
  ...
  const c = { input: "w-full bg-[#030303] border border-[#1a1a1a] rounded px-3 py-2 text-[#eaeaea] focus:border-[#666] focus:outline-none" }
      ...
      {published.themes?.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-4">
          {published.themes.map((t) => (
            <span key={t} className="text-xs px-2 py-1 rounded border border-[#1a1a1a] bg-[#030303] text-[#666]">{t}</span>
          ))}
        </div>
      )}
  ...
  return (
    <section id="review-dashboard" className="rounded-lg border border-[#1a1a1a] bg-[#0a0a0a] p-5">
      ...
      <div className="border-t border-[#1a1a1a] pt-4 space-y-3">
        ...
        >
          {generating ? (
            <><svg className="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg> Generating…</>
          ) : latest ? "Regenerate summary" : "Generate summary"}
        </button>
          ...
          <div className="rounded border border-[#1a1a1a] bg-[#030303] p-3">
            ...
            {latest.themes?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {latest.themes.map((t) => (
                  <span key={t} className="text-xs px-2 py-0.5 rounded border border-[#1a1a1a] text-[#666]">{t}</span>
                ))}
              </div>
            )}
            <button
...
export default function App() {
  ...
  return (
    <main id="app" className="min-h-screen bg-[#030303] text-[#eaeaea]" style={{ fontFamily: "Inter, sans-serif" }}>
      <header id="app-header" className="sticky top-0 z-10 border-b border-[#1a1a1a] bg-[#030303]/90 backdrop-blur px-5 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-medium tracking-tight" style={{ fontFamily: "'Space Mono', monospace" }}>Pulse Survey</h1>
