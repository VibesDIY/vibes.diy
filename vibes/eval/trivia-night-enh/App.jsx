...
function HostPanel({ database, currentRound, currentQuestion, questions }) {
  ...
  async function generateRound() {
    ...
    try {
      ...
      const roundResult = await database.put({ type: "round", topic: topic.trim(), currentIndex: 0, revealed: false, createdAt: Date.now() })
      for (let i = 0; i < data.questions.length; i++) {
        const q = data.questions[i]
        await database.put({ type: "question", roundId: roundResult.id, order: i, prompt: q.prompt, choices: q.choices, correctIndex: q.correctIndex, points: q.points })
      }
      setTopic("")
    } finally {
      setIsLoading(false)
  ...
  const spinner = <svg className="animate-spin w-4 h-4 inline" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeDasharray="50 50" /></svg>

  return (
    <section id="host-panel" className="rounded-xl border-2 border-[#f93c94] bg-[#2a0a2e]/80 p-4 shadow-[0_0_20px_rgba(249,60,148,0.4)] space-y-3">
      ...
      {(!currentRound || currentRound.revealed) && (
        <div className="space-y-2">
          <div className="flex gap-2">
            <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Topic (e.g. 80s movies)" className="flex-1 min-h-[44px] px-3 rounded-lg bg-[#4d1558] text-[#fcee0a] border-2 border-[#f93c94] font-['Rajdhani'] placeholder:text-[#00f0ff]/60" />
            <button onClick={suggestTopic} disabled={isLoading} className="min-h-[44px] px-3 bg-[#00f0ff] text-[#2a0a2e] font-['Orbitron'] font-bold rounded-lg disabled:opacity-50">{isLoading ? spinner : "AI"}</button>
          </div>
          <button onClick={generateRound} disabled={isLoading || !topic.trim()} className="w-full min-h-[44px] bg-[#f93c94] text-[#ffffff] font-['Orbitron'] font-bold rounded-lg disabled:opacity-50 tracking-wide">
      ...
      {currentRound && !currentRound.revealed && (
        <div className="space-y-2">
          <p className="font-['Share_Tech_Mono'] text-[#00f0ff] text-sm">Topic: {currentRound.topic} • Q{currentRound.currentIndex + 1}/{questions.length}</p>
          {currentQuestion && <p className="font-['Rajdhani'] text-[#fcee0a]">Correct: {String.fromCharCode(65 + currentQuestion.correctIndex)}</p>}
          <button onClick={advance} className="w-full min-h-[44px] bg-[#fcee0a] text-[#2a0a2e] font-['Orbitron'] font-bold rounded-lg tracking-wide">
...
function QuestionStage({ currentRound, currentQuestion }) {
  return (
    <section id="question-stage" className="rounded-xl border-2 border-[#00f0ff] bg-gradient-to-br from-[#4d1558] to-[#2a0a2e] p-6 shadow-[0_0_30px_rgba(0,240,255,0.3)] min-h-[200px]">
      {!currentRound && <p className="font-['Orbitron'] text-center text-[#fcee0a] text-xl mt-12">Waiting for the host…</p>}
      {currentRound && currentRound.revealed && <p className="font-['Orbitron'] text-center text-[#fcee0a] text-2xl mt-12">ROUND COMPLETE</p>}
      {currentRound && !currentRound.revealed && currentQuestion && (
        <div className="space-y-4">
          <p className="font-['Share_Tech_Mono'] text-[#00f0ff] text-sm">Question {currentRound.currentIndex + 1} • {currentQuestion.points} pts</p>
          ...
          {currentQuestion.choices && (
            <ul className="space-y-2 mt-3">
              {currentQuestion.choices.map((ch, i) => (
                <li key={i} className="font-['Rajdhani'] text-[#ffffff] text-lg border border-[#f93c94] rounded-lg px-3 py-2 bg-[#2a0a2e]/60">
                  <span className="text-[#f93c94] font-bold mr-2">{String.fromCharCode(65 + i)}.</span>{ch}
                </li>
              ))}
            </ul>
          )}
    ...
    <section id="player-answer" className="rounded-xl border-2 border-[#fcee0a] bg-[#2a0a2e]/80 p-4">
      ...
      {currentQuestion && !currentRound.revealed && (
        <div className="grid grid-cols-2 gap-2">
          {currentQuestion.choices.map((ch, i) => (
            <button key={i} onClick={() => submit(i)} disabled={!!myAnswer} className={`min-h-[56px] px-3 font-['Orbitron'] font-bold rounded-lg border-2 tracking-wide ${myAnswer?.choiceIndex === i ? 'bg-[#f93c94] text-[#ffffff] border-[#fcee0a]' : 'bg-[#4d1558] text-[#fcee0a] border-[#f93c94]'} disabled:opacity-60`}>
              {String.fromCharCode(65 + i)}
            </button>
          ))}
        </div>
      )}
      {myAnswer && !currentRound.revealed && <p className="font-['Share_Tech_Mono'] text-sm text-[#00f0ff] mt-2">✓ Locked in {String.fromCharCode(65 + myAnswer.choiceIndex)}</p>}
    </section>
  )
}
...
function Scoreboard({ currentRound, questions, answers }) {
  ...
  return (
    <section id="scoreboard" className="rounded-xl border-2 border-[#f93c94] bg-[#2a0a2e]/80 p-4 shadow-[0_0_25px_rgba(252,238,10,0.4)]">
      ...
      <ol className="space-y-2">
        {ranked.map(([handle, pts], i) => (
          <li key={handle} className={`flex items-center justify-between px-3 py-2 rounded-lg border ${i === 0 ? 'bg-[#fcee0a] text-[#2a0a2e] border-[#f93c94]' : 'bg-[#4d1558] text-[#fcee0a] border-[#00f0ff]'}`}>
  ...
  return (
    <div className={c.page}>
      ...
      <main id="app" className={c.main}>
        ...
        {isOwner && <HostPanel database={database} currentRound={currentRound} currentQuestion={currentQuestion} questions={sortedQuestions} />}
        {viewer && !isOwner && <PlayerAnswer viewer={viewer} database={database} currentRound={currentRound} currentQuestion={currentQuestion} answers={answers} />}
