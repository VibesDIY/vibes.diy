...
function Uploader({ viewer, activeAlbum, albums, database }) {
  ...
  async function upload(e) {
    ...
    try {
      let tags = []
      try {
        const res = await callAI(`Suggest 3-6 short tags for a trip photo in album "${albumName}" with caption "${caption || "(none)"}". Categories: location, activity, people, mood.`, {
...
function PhotoFeed({ photos, activeAlbum, isOwner, deletePhoto, ViewerTag }) {
  ...
  return (
    <section id="feed" className="px-4 py-4 bg-[oklch(0.93_0.03_130)] min-h-[60vh]">
      ...
      {filtered.length === 0 && <p className="text-sm text-[oklch(0.45_0.01_0)] italic">No photos yet — be the first to drop one.</p>}
      <ul id="photo-list" className="grid grid-cols-2 gap-3">
        {filtered.map(p => (
          <li key={p._id} className="bg-[oklch(0.97_0.01_80)] border-2 border-[oklch(0.12_0.01_0)] rounded p-2 shadow-[3px_3px_0_oklch(0.12_0.01_0)]">
            {p._files?.image?.url && (
            ...
            {p.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {p.tags.map((t, i) => (
                  <span key={i} className="text-xs px-2 py-0.5 bg-[oklch(0.90_0.05_240)] border border-[oklch(0.12_0.01_0)] rounded-full">{t}</span>
                ))}
              </div>
            )}
            <div className="flex items-center justify-between text-xs text-[oklch(0.45_0.01_0)]">
              ...
              {isOwner && (
                <button onClick={() => deletePhoto(p._id)} className="px-2 py-1 bg-[oklch(0.90_0.06_10)] border border-[oklch(0.12_0.01_0)] rounded text-[oklch(0.12_0.01_0)] font-semibold">
                  Remove
                </button>
              )}
            </div>
...
export default function App() {
  ...
  return (
    <main id="app" className="min-h-screen bg-[oklch(0.93_0.03_130)] font-['Inter']">
      <header id="app-header" className="px-4 py-3 bg-[oklch(0.90_0.06_10)] border-b-2 border-[oklch(0.12_0.01_0)] flex items-center justify-between sticky top-0 z-10">
        <h1 className="font-['Caveat'] text-3xl text-[oklch(0.12_0.01_0)]">Trip Board</h1>
        <ViewerTag />
      </header>
      <AlbumBar albums={albums} activeAlbum={activeAlbum} setActiveAlbum={setActiveAlbum} isOwner={isOwner} newAlbum={newAlbum} setNewAlbum={setNewAlbum} createAlbum={createAlbum} />
