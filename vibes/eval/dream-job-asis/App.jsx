...
function GalleryHero({ count }) {
  return (
    <section id="gallery-hero" className="px-5 pt-6 pb-4">
      <p className="font-display text-2xl leading-none" style={{ color: 'var(--muted)' }}>welcome to the</p>
      <h2 className="font-display text-5xl leading-tight" style={{ color: 'var(--ink)' }}>Dream Job Wall</h2>
...
function CreateSurface({ viewer, ViewerTag, onCreate }) {
  if (!viewer) {
    return (
      <section id="create-surface" className="mx-5 mb-4 p-4 rounded-2xl border-2 border-dashed" style={{ borderColor: 'var(--ink)', background: 'var(--yellow)' }}>
        <p className="font-display text-2xl" style={{ color: 'var(--ink)' }}>Sign in to pin your dream</p>
        <div className="mt-2"><ViewerTag /></div>
      </section>
    )
  }
  return (
...
function CreateModal({ open, onClose, viewer, database }) {
  ...
  React.useEffect(() => {
    if (!open) {
      setStep('camera'); setPhotoFile(null); setPhotoPreview(null)
      setJob(''); setComposed(null); setIsLoading(false)
    }
  }, [open])
  ...
  return (
    <div className={c.overlay} style={{ background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
      <div className={c.sheet} style={{ borderColor: 'var(--ink)', background: 'var(--paper)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={c.body}>
          {step === 'camera' && (
            <>
              ...
              <button onClick={() => fileRef.current?.click()} className={c.btnPrimary} style={{ borderColor: 'var(--ink)', background: 'var(--yellow)', color: 'var(--ink)' }}>
                Open camera
              </button>
            </>
          )}
          {step === 'job' && (
            <>
              {photoPreview && <img src={photoPreview} alt="selfie" className="w-32 h-32 rounded-2xl object-cover border-2 mx-auto" style={{ borderColor: 'var(--ink)' }} />}
              <label className={c.label} style={{ color: 'var(--ink)' }}>Your dream job?</label>
              <input value={job} onChange={(e) => setJob(e.target.value)} placeholder="e.g. deep-sea jellyfish DJ" className={c.input} style={{ borderColor: 'var(--ink)', color: 'var(--ink)' }} />
              <div className="flex gap-2">
                <button onClick={suggestJob} disabled={suggesting} className={c.btnGhost} style={{ borderColor: 'var(--ink)', background: 'var(--pink)', color: 'var(--ink)' }}>
                  {suggesting ? (
                    <svg className="animate-spin inline" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>
                  ) : 'Suggest one'}
                </button>
                <button onClick={compose} disabled={isLoading || !job.trim()} className="flex-1 min-h-[44px] rounded-xl border-2 font-display text-xl" style={{ borderColor: 'var(--ink)', background: 'var(--yellow)', color: 'var(--ink)' }}>
                  {isLoading ? (
                    <span className="flex items-center justify-center gap-2">
                      <svg className="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>
                      Composing…
                    </span>
                  ) : 'Compose →'}
                </button>
          ...
          {step === 'preview' && composed && (
            <>
              ...
              <p className="font-display text-2xl" style={{ color: 'var(--ink)' }}>"{composed.caption}"</p>
              <p className="text-sm" style={{ color: 'var(--muted)' }}>Job: {job}</p>
              <button onClick={post} className={c.btnPrimary} style={{ borderColor: 'var(--ink)', background: 'var(--pink)', color: 'var(--ink)' }}>
                Pin to the wall
              </button>
            </>
          )}
...
function PortraitCard({ doc, ViewerTag }) {
  ...
  return (
    <article className={`break-inside-avoid mb-4 p-3 border-2 shadow-[4px_4px_0_var(--ink)] ${rot}`} style={{ borderColor: 'var(--ink)', background: tint }}>
      ...
      <p className="font-display text-2xl mt-3 leading-tight" style={{ color: 'var(--ink)' }}>"{doc.caption}"</p>
      <p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>dreams of being a {doc.job}</p>
      <div className="mt-2"><ViewerTag userHandle={doc.authorHandle} /></div>
    </article>
  )
}
...
function Gallery({ docs, ViewerTag }) {
    ...
    return (
      <section id="gallery" className="px-5 pb-24">
        <div className="p-8 rounded-2xl border-2 border-dashed text-center" style={{ borderColor: 'var(--muted)' }}>
          <p className="font-display text-3xl" style={{ color: 'var(--muted)' }}>The wall is empty</p>
          <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>Be the first to pin a dream</p>
        </div>
      </section>
    )
  }
  return (
    <section id="gallery" className="px-5 pb-24 columns-2 md:columns-3 gap-4">
      {docs.map((d) => <PortraitCard key={d._id} doc={d} ViewerTag={ViewerTag} />)}
    </section>
  )
}
...
export default function App() {
  ...
  return (
    <main id="app" className="min-h-screen" style={{ background: 'var(--desk)' }}>
      <ThemeStyles />
      <header id="app-header" className="px-5 py-4 flex items-center justify-between border-b-2" style={{ borderColor: 'var(--ink)', background: 'var(--paper)' }}>
        <h1 className="font-display text-4xl" style={{ color: 'var(--ink)' }}>Dream Jobs</h1>
        <ViewerTag />
      </header>
