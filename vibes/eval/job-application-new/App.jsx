...
function ApplicationForm({ database, viewer }) {
  ...
  const c = {
    ...
    input: "w-full border border-[#cccccc] bg-white px-3 py-3 text-[#111111] min-h-[44px] focus:outline-none focus:border-[#111111]",
    textarea: "w-full border border-[#cccccc] bg-white px-3 py-2 text-[#111111] min-h-[120px] focus:outline-none focus:border-[#111111]",
    button: "w-full min-h-[44px] bg-[#111111] text-white font-medium px-4 py-3 disabled:opacity-50 flex items-center justify-center gap-2",
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className={c.label} htmlFor="letter">Cover letter</label>
            <button type="button" onClick={suggestLetter} disabled={isLoading} className={c.suggest}>Suggest example</button>
          </div>
          ...
          <li key={a._id} className={c.card}>
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              {typeof a.analysis?.score === "number" && (
                <span className={c.score}>{Math.round(a.analysis.score)}/100</span>
              )}
            </div>
            {a.analysis?.summary && (
              <p className="text-sm text-[#111111] mb-2 italic">{a.analysis.summary}</p>
            )}
