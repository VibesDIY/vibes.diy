...
function Arena({ viewer }) {
  ...
  return (
    <section id="arena" className="rounded-2xl border-2 border-[#f93c94] bg-[#2a0a2e]/80 p-4 shadow-[0_0_30px_rgba(249,60,148,0.4)]">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-[Orbitron] text-xl text-[#fcee0a] tracking-widest">ARENA</h2>
        <span className="font-[Share_Tech_Mono] text-[#00f0ff] text-lg">SCORE: <span className="text-[#fcee0a] font-bold">{score}</span></span>
      </div>
      <div className="aspect-square w-full max-w-md mx-auto bg-[#1a0520] border-2 border-[#f93c94]/50 rounded-lg relative overflow-hidden" style={{ boxShadow: "inset 0 0 40px rgba(249,60,148,0.3)" }}>
        <div className="absolute inset-0 grid" style={{ gridTemplateColumns: `repeat(${GRID}, 1fr)`, gridTemplateRows: `repeat(${GRID}, 1fr)` }}>
          {snake.map((s, i) => (
            <div key={i} style={{ gridColumn: s.x + 1, gridRow: s.y + 1, background: i === 0 ? "#fcee0a" : "#f93c94", boxShadow: i === 0 ? "0 0 10px #fcee0a" : "0 0 8px #f93c94", borderRadius: "20%" }} />
          ))}
          <div style={{ gridColumn: food.x + 1, gridRow: food.y + 1, background: "#00f0ff", boxShadow: "0 0 12px #00f0ff", borderRadius: "50%" }} />
        </div>
        {!running && (
          <div className="absolute inset-0 flex items-center justify-center bg-[#2a0a2e]/80">
            <div className="text-center">
              {gameOver && <p className="font-[Orbitron] text-2xl text-[#f93c94] mb-2 drop-shadow-[0_0_8px_rgba(249,60,148,0.8)]">GAME OVER</p>}
              {gameOver && <p className="font-[Share_Tech_Mono] text-[#fcee0a] mb-3">final: {score}</p>}
              {viewer ? (
                <button onClick={start} className="font-[Orbitron] tracking-widest bg-[#fcee0a] text-[#2a0a2e] px-6 py-3 rounded-lg min-h-[44px] shadow-[0_0_20px_rgba(252,238,10,0.6)] hover:scale-105 transition">
      ...
      <div className="grid grid-cols-3 gap-2 mt-4 max-w-[200px] mx-auto md:hidden">
        <div />
        <button onClick={() => press({ x: 0, y: -1 })} className="min-h-[44px] bg-[#4d1558] border-2 border-[#f93c94] text-[#fcee0a] rounded-lg font-bold">↑</button>
        <div />
        <button onClick={() => press({ x: -1, y: 0 })} className="min-h-[44px] bg-[#4d1558] border-2 border-[#f93c94] text-[#fcee0a] rounded-lg font-bold">←</button>
        <button onClick={() => press({ x: 0, y: 1 })} className="min-h-[44px] bg-[#4d1558] border-2 border-[#f93c94] text-[#fcee0a] rounded-lg font-bold">↓</button>
        <button onClick={() => press({ x: 1, y: 0 })} className="min-h-[44px] bg-[#4d1558] border-2 border-[#f93c94] text-[#fcee0a] rounded-lg font-bold">→</button>
      </div>
    </section>
...
function Leaderboard() {
  return (
    <section id="leaderboard" className="rounded-2xl border-2 border-[#00f0ff] bg-[#4d1558]/80 p-4 shadow-[0_0_30px_rgba(0,240,255,0.4)]">
...
export default function App() {
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 backdrop-blur-md bg-[#2a0a2e]/70 border-b-2 border-[#f93c94] px-4 py-3 flex items-center justify-between",
