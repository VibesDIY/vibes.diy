...
function AddBookmark({ isOwner, database }) {
  ...
  if (!isOwner) {
    return (
      <section id="add-bookmark" className="rounded-2xl border border-[oklch(0.65_0.15_80/0.12)] bg-[oklch(0.12_0.03_280/0.7)] p-4 backdrop-blur">
  ...
  return (
    <section id="add-bookmark" className="rounded-2xl border border-[oklch(0.65_0.15_80/0.12)] bg-[oklch(0.12_0.03_280/0.7)] p-4 backdrop-blur">
      <div className="flex items-center justify-between mb-3">
        ...
        <AISuggestButton onPick={({ url: u, title: t }) => { setUrl(u); setTitle(t) }} disabled={saving} />
      </div>
      <form onSubmit={save} className="space-y-2">
        ...
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="title (optional — AI will infer)"
          className="w-full min-h-[44px] px-3 rounded-lg bg-[oklch(0.08_0.03_280)] border border-[oklch(0.65_0.15_80/0.12)] text-[oklch(0.93_0.02_80)] placeholder:text-[oklch(0.50_0.04_290)] focus:border-[oklch(0.72_0.15_75)] outline-none" />
        <button type="submit" disabled={saving || !url.trim()}
          ...
          {saving && <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>}
...
function TagCloud({ tags, activeTag, onPick }) {
  ...
  return (
    <section id="tag-cloud" className="rounded-2xl border border-[oklch(0.65_0.15_80/0.12)] bg-[oklch(0.12_0.03_280/0.7)] p-4 backdrop-blur">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {tags.filter(([t]) => t !== activeTag).map(([tag, count]) => (
...
function BookmarkFeed({ docs, isOwner, database, query, setQuery, activeTag }) {
  ...
  return (
    <section id="bookmark-feed" className="rounded-2xl border border-[oklch(0.65_0.15_80/0.12)] bg-[oklch(0.12_0.03_280/0.7)] p-4 backdrop-blur space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-mono text-xs uppercase tracking-widest text-[oklch(0.72_0.15_75)]">// archive ({filtered.length})</h2>
      </div>
      <input value={query} onChange={e => setQuery(e.target.value)} placeholder="search titles, tags, urls…"
        className="w-full min-h-[44px] px-3 rounded-lg bg-[oklch(0.08_0.03_280)] border border-[oklch(0.65_0.15_80/0.12)] text-[oklch(0.93_0.02_80)] placeholder:text-[oklch(0.50_0.04_290)] focus:border-[oklch(0.72_0.15_75)] outline-none" />
      <ul className="space-y-2">
        ...
        {filtered.map(d => (
          <li key={d._id} className="rounded-xl border border-[oklch(0.65_0.15_80/0.12)] p-3 bg-[oklch(0.08_0.03_280)] hover:border-[oklch(0.72_0.15_75/0.4)]">
            <div className="flex items-start justify-between gap-2">
              <a href={d.url} target="_blank" rel="noreferrer" className="text-[oklch(0.72_0.15_75)] font-medium leading-snug break-words flex-1">
                {d.title}
              </a>
              {isOwner && (
                <button onClick={() => database.del(d._id)} aria-label="delete"
            ...
            <div className="mt-2 flex flex-wrap gap-1.5 items-center">
              <span className="text-[10px] font-mono text-[oklch(0.50_0.04_290)] truncate max-w-[180px]">{new URL(d.url).hostname}</span>
              {(d.tags || []).map(t => (
                <span key={t} className="text-[10px] font-mono text-[oklch(0.55_0.18_300)]">#{t}</span>
              ))}
            </div>
          </li>
        ))}
...
function AISuggestButton({ onPick, disabled }) {
  ...
  return (
    <button type="button" onClick={suggest} disabled={disabled || busy}
      ...
      <main id="app" className={c.wrap}>
        <header id="app-header" className={c.header}>
          <div>
            ...
            <p className={c.sub}>{docs.length} saved · {isOwner ? "owner mode" : "read-only"}</p>
          </div>
          <ViewerTag />
        </header>
