...
function RecipeComposer({ viewer, database }) {
  ...
  async function suggest() {
    setSuggesting(true)
    try {
      const res = await callAI("Invent one playful emoji-only recipe. Return a short title and an emoji sequence (6-10 emojis, no text).", {
  ...
  const c = {
    ...
    input: "w-full mt-3 bg-[oklch(0.17_0.000_0)] border border-[oklch(0.37_0.03_260)] rounded px-3 py-2 text-[oklch(0.93_0.006_265)]",
    row: "flex gap-2 mt-3",
    primary: "flex-1 min-h-[44px] rounded bg-[oklch(0.93_0.006_265)] text-[oklch(0.17_0.000_0)] font-semibold disabled:opacity-50",
    ghost: "min-h-[44px] px-3 rounded border border-[oklch(0.37_0.03_260)] text-[oklch(0.93_0.006_265)] text-sm disabled:opacity-50",
  ...
  return (
    <section id="recipe-composer" className={c.wrap}>
      ...
      <div className={c.palette}>
        {PALETTE.map((e,i) => (
          <button key={i} onClick={() => setSeq(s => [...s, e])} className={c.chip}>{e}</button>
        ))}
      </div>
      <div className={c.row}>
        <button onClick={() => setSeq(s => s.slice(0,-1))} className={c.ghost}>⌫</button>
        <button onClick={() => setSeq([])} className={c.ghost}>clear</button>
      </div>
...
function RecipeCard({ recipe, viewer, isOwner, database, votes, roasts, isPinned, onPin }) {
  ...
  async function toggleVote() {
    if (!viewer) return
    if (myVote) { await database.del(myVote._id) }
    else { await database.put({ type: "vote", recipeId: recipe._id, authorHandle: viewer.userHandle, createdAt: Date.now() }) }
  }
  ...
  async function roastIt() {
    ...
    setRoasting(true)
    try {
      const res = await callAI(`You are a snarky food critic. Roast this emoji recipe: ${recipe.emojis.join(" ")} (title: "${recipe.title}"). Be witty, not cruel.`, {
        schema: { properties: {
  ...
  return (
    <article className={c.card}>
      ...
      <div className={c.meta}>
        <span>by</span>
        <span className="not-italic"><ViewerTagInline handle={recipe.authorHandle} /></span>
      </div>
...
function RecipeFeed({ viewer, isOwner, database, recipes, votes, roasts, pinnedId }) {
  ...
  return (
    <section id="recipe-feed" className="space-y-3">
      ...
      {unpinned.map(r => (
        <RecipeCard key={r._id} recipe={r} viewer={viewer} isOwner={isOwner} database={database}
          votes={votes} roasts={roasts} isPinned={false} onPin={pin} />
      ))}
    </section>
  )
}
...
export default function App() {
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 bg-[oklch(0.17_0.000_0)]/95 backdrop-blur border-b border-[oklch(0.37_0.03_260)] px-4 py-3 flex items-center justify-between",
