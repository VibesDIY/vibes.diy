    ...
    <section id="live-bullets" className={c.frame}>
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-[#666666] mt-1 flex items-center gap-2">
                ...
                {viewer && d.authorHandle === viewer.userHandle && (
                  <button className={c.delBtn} onClick={() => database.del(d._id)}>delete</button>
                )}
              </div>
            </div>
          </li>
      ...
      {viewer ? (
        <form onSubmit={onSubmit} className="flex gap-2 mt-3">
          <input className={c.input} value={doc.text} onChange={(e) => merge({ text: e.target.value })} placeholder="Add a bullet point…" />
...
function SummaryPanel({ database, useLiveQuery, isOwner, viewer }) {
  ...
  return (
    <section id="summary-panel" className={c.frame}>
      <h2 className={c.heading}>Summary</h2>
      <p className={c.muted}>{bullets.length} bullet{bullets.length === 1 ? "" : "s"} ready to summarize.</p>
    ...
    <section id="summary-archive" className={c.frame}>
      ...
      {sorted.map((s) => (
        <div key={s._id} className={c.card}>
          <div className={c.date}>{new Date(s.createdAt).toLocaleString()} · {s.bulletCount} bullets</div>
          {s.decisions?.length > 0 && (
            <>
              <div className={c.label}>Decisions</div>
              <ul>{s.decisions.map((d, i) => <li key={i} className={c.item}>{d}</li>)}</ul>
            </>
          )}
          {s.actionItems?.length > 0 && (
            <>
              <div className={c.label}>Action items</div>
              <ul>{s.actionItems.map((d, i) => <li key={i} className={c.item}>{d}</li>)}</ul>
            </>
          )}
          {s.openQuestions?.length > 0 && (
            <>
              <div className={c.label}>Open questions</div>
              <ul>{s.openQuestions.map((d, i) => <li key={i} className={c.item}>{d}</li>)}</ul>
            </>
          )}
        </div>
      ))}
  ...
  return (
    <div className={c.page}>
      ...
      <main id="app" className={c.main}>
        <LiveBullets database={database} useLiveQuery={useLiveQuery} useDocument={useDocument} viewer={viewer} ViewerTag={ViewerTag} />
