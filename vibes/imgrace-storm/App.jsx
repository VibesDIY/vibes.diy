...
// ── Contention config (this app: faithful hat-smeller racer) ────────
// Key ingredient: access.js runs QuickJS eval on every put. The hat doc
// (type:"hat") gets channels:["cabinet"]; the image doc (type:"image")
// gets {} — and the immediate get races the channel/grant assignment.
const CONFIG = {
  title: "Image Storm",
  slots: 1, // one image at a time, like a user submitting one hat
  promptStyle: "long", // long generation is the documented trigger
  noiseWriter: false, // noise + access.js overloads QuickJS → put timeouts, not the race
  noiseEveryMs: 70,
  companionWrites: true, // channeled hat doc, then the {} image doc (hat-smeller shape)
};
...
export default function ImgRaceApp() {
  ...
  useEffect(() => {
    ...
    let n = 0;
    const t = setInterval(() => {
      noiseDb.put({ _id: `noise-${runId}-${n++}`, type: "noise", payload: "x".repeat(2500), created: Date.now() }).catch(() => {});
    }, CONFIG.noiseEveryMs);
    return () => clearInterval(t);
  }, [running, runId, noiseDb]);

  const onDone = (idx, status, message) => setResults((p) => (p[idx] ? p : { ...p, [idx]: { status, message } }));
  ...
  return (
    <div style={{ fontFamily: "monospace", padding: 24, background: "#0f0f23", minHeight: "100vh", color: "#e0e0e0" }}>
      ...
      <p style={{ color: "#888", fontSize: 13 }}>
        {CONFIG.slots} concurrent generations × {BATCHES} batches, long prompts, write-noise on the same AppSessions DO during the
        image puts. Watches each slot for "Failed to get document: not-found" (the race).
      </p>
      <button
        onClick={run}
        disabled={active}
...
function Slot({ runId, idx, onDone, result }) {
    ...
    if (CONFIG.companionWrites && !companionRef.current) {
      ...
      parentDb
        .put({
          ...
          name: "Storm Hat",
          material: "cloud silk",
          description: "a towering thundercloud hat",
          created: Date.now(),
        })
        .catch(() => {});
    }
  ...
  const color =
    result?.status === "race" ? "#ff5577" : result?.status === "ok" ? "#22c55e" : result?.status === "err" ? "#f59e0b" : "#555";
  return (
