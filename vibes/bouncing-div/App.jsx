...
function BouncingStage({ settings }) {
  ...
  React.useEffect(() => {
    ...
    function tick() {
      ...
      if (s.x + size >= w) { s.x = w - size; s.vx = -Math.abs(s.vx) }
      if (s.y + size >= h) { s.y = h - size; s.vy = -Math.abs(s.vy) }
      setPos({ x: s.x, y: s.y })
  ...
  const c = {
    stage: "relative w-full h-[320px] md:h-[420px] rounded-[var(--radius-lg)] border-[length:var(--border-width)] border-[var(--border)] bg-[var(--surface)] overflow-hidden",
  }
...
function Controls({ settings, database, canEdit }) {
  ...
  const c = {
    panel: "p-[var(--spacing)] rounded-[var(--radius-lg)] border-[length:var(--border-width)] border-[var(--border)] bg-[var(--surface)] space-y-3",
    ...
    secondary: "min-h-[44px] px-4 py-2 rounded-[var(--radius)] border-[length:var(--border-width)] border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] font-[var(--font-family)]",
    range: "w-full",
    ...
    return (
      <div className={c.panel}>
        ...
        <div className={c.row}>
          ...
          <span className={c.value} style={{ color: settings.color }}>{settings.color}</span>
        </div>
        <div className={c.row}>
  ...
  return (
    <div className={c.panel}>
      <div className={c.row}>
        <label className={c.label} htmlFor="color">Color</label>
        <input
          id="color"
          type="color"
      ...
      <div>
        ...
        <input
          type="range" min="1" max="20" step="1" value={settings.speed}
      ...
      <div>
        ...
        <input
          type="range" min="30" max="160" step="2" value={settings.size}
      ...
      <button className={c.btn} onClick={surprise} disabled={aiLoading}>
        {aiLoading ? <><Spinner /> Thinking...</> : "Surprise me"}
      </button>
...
export default function App() {
  ...
  const c = {
    ...
    header: "px-4 py-3 border-b-[length:var(--border-width)] border-[var(--border)] bg-[var(--surface)] flex items-center justify-between",
