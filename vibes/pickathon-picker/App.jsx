...
const festivalDayFor = (dateStr) => {
  ...
  const partsFmt = new Intl.DateTimeFormat("en-US", {
    timeZone: FESTIVAL_TZ, weekday: "long", hourCycle: "h23", hour: "2-digit",
  })
...
function ScheduleView({ days, getDateForDay, buildSchedule, fmtTime, notes, c, shiftStartRaw, shiftEndRaw, emptyMessage, eventNotes, savingNotes, onNoteChange, onNoteBlur, onNoteFocus, canWrite, focusedNote, onToggleFavorite, myFavIds }) {
      ...
      {days.map(day => {
        ...
        return (
          <div key={day} className={c.schedDay}>
            <h3 className="text-xl font-black mb-4 text-white">{day} — {getDateForDay(day)}</h3>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          {item.type === "event" && onToggleFavorite && (
                            <button onClick={() => onToggleFavorite(item.data)} className="p-1 bg-[#CD6C0C] text-white rounded-lg border-2 border-[#4A4A4A] text-xs font-bold px-2">
...
export default function PickathonPicker() {
  ...
  const fetchSchedule = async () => {
    ...
    if (cached && !cached.isStale) { ingest(cached.data); setLoading(false); return }
    if (cached && cached.isStale) { ingest(cached.data); setLoading(false) }
    try {
      ...
      console.error(e)
      if (cached) { setError("Using cached data"); ingest(cached.data) }
  ...
  const fmtDate = (s) => toDate(s).toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric", timeZone: FESTIVAL_TZ })
  ...
  const getDateForDay = (day) => {
    ...
    const d = new Date(base); d.setDate(base.getDate() + Math.max(0, idx))
  ...
  useEffect(() => {
    const newEventNotes = {}, newOriginalNotes = {}
    notesDocs.forEach(n => {
  ...
  const { doc: shiftForm, merge: mergeShift, reset: resetShift } =
  ...
  const deleteShift = async (shiftId) => { await database.del(shiftId) }
  ...
  const bandsList = useMemo(() => {
    ...
    for (const e of events) {
      const key = e.title
      if (!map.has(key)) map.set(key, { title: key, url: e.url, events: [] })
      map.get(key).events.push(e)
    }
    for (const b of map.values()) b.events.sort((a, b) => toDate(a.start) - toDate(b.start))
  ...
  const c = {
    ...
    navBtn: (active) => `px-6 py-3 font-bold rounded-2xl border-4 border-[#4A4A4A] transition-all ${active ? "bg-[#4A4A4A] text-white" : "bg-white text-[#4A4A4A] hover:bg-[#BACD32]"}`,
    ...
    return (
      <div className={`min-h-screen ${c.pageBg} p-4`}>
        <div className={`max-w-4xl mx-auto ${c.cardBg} rounded-3xl shadow-2xl border-8 ${c.border} p-8`}>
          ...
          <button onClick={fetchSchedule} className={c.btnPink}>Retry</button>
        </div>
      </div>
    )
  }
        ...
        <div className={`${c.headerBg} border-b-8 ${c.border} p-6`}>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className={`text-4xl font-black ${c.bodyText} mb-1`}>{superMode ? "SUPER PICKATHON PICKER" : "PICKATHON PICKER"}</h1>
                <p className={`${c.bodyText} text-base font-bold`}>Jul 30 – Aug 2, 2026 · Pendarvis Farm, Happy Valley, OR</p>
                <p className={`${c.bodyText} text-sm mt-1`}><em>Discover, favorite, and organize your perfect festival experience</em></p>
              </div>
            </div>
            <ViewerTag />
          </div>
          ...
          {!canWrite && (
            <div className={c.readOnlyBanner}>Sign in to save your favorites.</div>
          )}
        </div>
        ...
        <div className={`${c.navBg} border-b-8 ${c.border} p-4`}>
          <div className="flex flex-wrap gap-3">
            {["now", "browse", "bands", "favorites", "friends", "shifts", "schedule"]
              .filter(v => v === "now" || v === "browse" || v === "bands" || (v === "favorites" ? superMode && canWrite : canWrite))
              .map(viewName => (
            ...
            <div>
              <div className="mb-6 flex flex-wrap gap-4">
                ...
                >
                  ...
                  {displayDays.map(day => <option key={day} value={day}>{day}</option>)}
                </select>
              </div>
                  ...
                  <div key={event.eventId} className={myFavIds.has(event.eventId) ? c.favCard : c.eventCard}>
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          {superMode && favCounts[event.eventId] > 0 && (
                            <span className={c.badge} title="People who picked this">★ {favCounts[event.eventId]}</span>
                          )}
                        ...
                        <div className={`space-y-1 text-sm font-bold ${c.bodyText}`}>
                          ...
                          <p>{fmtTime(event.start)} – {fmtTime(event.end)}</p>
                        </div>
                        {canWrite ? (() => {
                      ...
                      <div className="flex gap-2">
                        {canWrite && (
                          <button onClick={() => toggleFavorite(event)} className={myFavIds.has(event.eventId) ? c.favToggleOn : c.favToggleOff}>
                        ...
                        <a href={event.url} target="_blank" rel="noopener noreferrer" className={c.linkBtn} title="View artist page">
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#4A4A4A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
                        </a>
                      </div>
                    </div>
                  </div>
          ...
          {view === "now" && (
            <div>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                ...
                <p className={`text-sm font-bold ${c.bodyText}`}>
                  {new Date(nowTick).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", timeZone: FESTIVAL_TZ })} festival time
                </p>
              </div>
              ...
              {nowSets.length === 0 ? (
                ...
                <div className="grid gap-3 mb-8">
                  {nowSets.map(event => {
                    ...
                    return (
                      <div key={event.eventId} className={isMine ? c.favCard : isFriendPick ? "bg-[#FFE680] rounded-2xl border-4 border-[#4A4A4A] p-4 shadow-lg" : c.eventCard}>
                        <div className="flex justify-between items-start gap-3 flex-wrap">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <h4 className={`text-lg font-black ${isMine ? "text-white" : c.bodyText}`}>{event.title}</h4>
                              {isFriendPick && !isMine && <span className={c.badge} title="A friend favorited this">friend pick</span>}
                            </div>
                  ...
                  return (
                    <div key={band.title} className={anyFav ? c.favCard : c.eventCard}>
                      <div className="flex justify-between items-start gap-3 flex-wrap">
                          ...
                          <div className="space-y-1">
                            {band.events.map(e => (
                              <div key={e.eventId} className="flex items-center gap-2 flex-wrap">
                                ...
                                {canWrite && (
                                  <button onClick={() => toggleFavorite(e)} className={myFavIds.has(e.eventId) ? c.favToggleOn : c.favToggleOff}>
                        ...
                        <a href={band.url} target="_blank" rel="noopener noreferrer" className={c.linkBtn} title="View artist page">
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#4A4A4A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
                        </a>
                      </div>
                    </div>
            ...
            <div>
              <div className="mb-6 p-4 bg-[#BACD32] rounded-2xl border-4 border-[#4A4A4A]">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  ...
                  {viewingUser && (
                    <button onClick={() => setViewingUser(null)} className={c.btnCyan}>Back to my picks</button>
                  )}
                </div>
                  ...
                  {favoriteEvents.map(event => (
                    <div key={event.eventId} className={c.favCard}>
                      <div className="flex justify-between items-start">
                        ...
                        {canWrite && (
                          <button onClick={() => toggleFavorite(event)} className="p-2 bg-white rounded-2xl border-2 border-[#4A4A4A] hover:bg-[#BACD32] transition-all ml-3">
          ...
          {view === "friends" && (
            <div>
              <div className="flex flex-col items-center gap-4 p-6 bg-[#BACD32] rounded-2xl border-4 border-[#4A4A4A] mb-6">
                ...
                <a href={connectUrl} target="_blank" rel="noopener noreferrer" className={c.btnPink}>Connect</a>
              ...
              {selectedFriend && (
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    ...
                    <button onClick={() => setSelectedFriend(null)} className={c.btnCyan}>Close</button>
                  </div>
                  <ScheduleView
                    days={displayDays}
                ...
                <div className={c.shiftForm}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <select value={shiftForm.day} onChange={e => mergeShift({ day: e.target.value })} className={c.input}>
                      {displayDays.map(day => (
                        <option key={day} value={day}>{day} ({getDateForDay(day)})</option>
                      ))}
                    </select>
                    <input type="time" value={shiftForm.startTime} onChange={e => mergeShift({ startTime: e.target.value })} className={c.input} />
                    <input type="time" value={shiftForm.endTime} onChange={e => mergeShift({ endTime: e.target.value })} className={c.input} />
                    <input type="text" placeholder="Shift, Meal, Break, …" value={shiftForm.kind || ""} onChange={e => mergeShift({ kind: e.target.value })} className={c.input} />
                  </div>
                  ...
                  <button onClick={submitShift} className={`mt-4 ${c.btnCyan}`}>Add Extra</button>
                </div>
              )}
                    ...
                    <div key={shift._id} className={c.shiftCard}>
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className={`font-bold ${c.bodyText}`}>{shift.day} ({getDateForDay(shift.day)}) — {fmtTime(shiftStart)} to {fmtTime(shiftEnd)}</p>
                          {canWrite && (
                        ...
                        {canWrite && (
                          <button onClick={() => deleteShift(shift._id)} className={c.deleteBtn} title="Delete shift">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" /></svg>
                          </button>
                        )}
                      </div>
                    </div>
