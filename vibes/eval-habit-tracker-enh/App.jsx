                ...
                return (
                  <li key={h._id} className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className={c.muted}>{streak} day streak {didToday ? "✓ today" : ""} · {cheerCount} cheers</div>
                    </div>
                    {viewer && viewer.userHandle !== handle && (
                      <button onClick={() => sendCheer(handle, h._id)} className={c.btnAlt}>Cheer</button>
                    )}
                  </li>
                )
              })}
...
export default function App() {
  ...
  const c = {
    ...
    header: "sticky top-0 z-10 backdrop-blur bg-[oklch(0.18_0.10_300/0.7)] border-b border-white/10 px-4 py-3 flex items-center justify-between",
  ...
  return (
    <div className={c.page}>
      ...
      <main id="app" className={c.shell}>
        ...
        <StreakBoardSection c={c} viewer={viewer} database={database} habits={habits} checkins={checkins} cheers={cheers} ViewerTag={ViewerTag} />
