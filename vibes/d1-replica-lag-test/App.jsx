...
export default function D1ReplicaLagTest() {
  ...
  const add = (r) => {
    ...
    console.log(
      `[D1TEST] ${r.pass ? "PASS" : "FAIL"} ${r.test} ${JSON.stringify(
  ...
  async function runTest() {
    ...
    try {
      ...
      const results = await Promise.allSettled([db1.get(id1), db2.get(id2), db3.get(id3)]);
      const tGet = performance.now();
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#666", fontSize: 12, marginBottom: 24 }}>
        Writes to multiple Firefly DBs on the same DO (same vibe = same AppSessions shard), then immediately reads back. If D1
        routes reads to a replica that hasn't caught the write, the get returns not-found.
      </p>
      ...
      {results.map((r, i) => (
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <span style={{ color: r.pass ? passColor : failColor, fontWeight: "bold" }}>{r.pass ? "PASS" : "FAIL"}</span>
          </div>
