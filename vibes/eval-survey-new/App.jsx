...
function SurveyForm({ database, onSubmitted, viewer }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try { localStorage.setItem("pulse:submitted", ok.id) } catch {}
      onSubmitted(ok.id)
  ...
  return (
    <section id="survey" className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {QUESTIONS.map((q, i) => (
          <div key={q.id}>
            <label className="block text-sm mb-2"><span className="text-[#666] font-mono mr-2">0{i+1}</span>{q.label}</label>
            {q.type === "rating" ? (
              <div className="flex gap-2">
                {[1,2,3,4,5].map(n => (
                  <button type="button" key={n} onClick={() => setAnswers({...answers, [q.id]: n})}
                    className={`min-h-[44px] flex-1 rounded-lg border ${answers[q.id]===n ? "bg-white text-black border-white" : "border-[#1a1a1a] text-[#eaeaea]"}`}>{n}</button>
                ))}
              </div>
            ) : (
              <textarea value={answers[q.id] || ""} onChange={e => setAnswers({...answers, [q.id]: e.target.value})}
                rows={2} className="w-full bg-[#030303] border border-[#1a1a1a] rounded-lg px-3 py-2 text-sm" />
            )}
          </div>
        ))}
        <button type="submit" disabled={submitting} className="w-full min-h-[44px] rounded-lg bg-white text-black font-medium disabled:opacity-50 flex items-center justify-center gap-2">
          {submitting && <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>}
...
function ThankYou() {
  return (
    <section id="thanks" className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-8 text-center">
      <svg className="mx-auto mb-3" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
    ...
    <section id="summary" className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-5">
      ...
      {summary.themes?.length > 0 && (
        <div className="mb-4">
          ...
          <ul className="space-y-1 text-sm">{summary.themes.map((t,i) => <li key={i}>• {t}</li>)}</ul>
        </div>
      )}
      ...
      {summary.quotes?.length > 0 && (
        <div>
          ...
          <ul className="space-y-2">{summary.quotes.map((q,i) => <li key={i} className="border-l-2 border-[#1a1a1a] pl-3 text-sm italic text-[#eaeaea]">"{q}"</li>)}</ul>
        </div>
      )}
    </section>
  )
...
function TeamDashboard({ database, useLiveQuery, summaries }) {
  ...
  const generateSummary = async () => {
    setGenerating(true)
    try {
      const prompt = `Analyze these ${responses.length} customer feedback responses and produce a summary.\n\n` +
  ...
  return (
    <section id="dashboard" className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-5">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button onClick={generateSummary} disabled={generating || responses.length===0}
          className="min-h-[44px] px-4 rounded-lg bg-white text-black text-sm font-medium disabled:opacity-50 flex items-center gap-2">
          {generating && <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg>}
          {generating ? "Generating..." : draft ? "Regenerate draft" : "Generate AI summary"}
        </button>
        {draft && <button onClick={publish} className="min-h-[44px] px-4 rounded-lg border border-white text-sm">Publish</button>}
        {published && <button onClick={unpublish} className="min-h-[44px] px-4 rounded-lg border border-[#1a1a1a] text-sm text-[#666]">Unpublish</button>}
      </div>
      ...
      {draft && (
        <div className="space-y-3 border-t border-[#1a1a1a] pt-4">
          ...
          <input value={draft.title || ""} onChange={e => editField("title", e.target.value)}
            placeholder="Title" className="w-full bg-[#030303] border border-[#1a1a1a] rounded-lg px-3 py-2 text-sm" />
          <textarea value={draft.sentiment || ""} onChange={e => editField("sentiment", e.target.value)}
            rows={3} placeholder="Sentiment summary" className="w-full bg-[#030303] border border-[#1a1a1a] rounded-lg px-3 py-2 text-sm" />
          <textarea value={(draft.themes || []).join("\n")} onChange={e => editField("themes", e.target.value.split("\n").filter(Boolean))}
            rows={3} placeholder="Themes (one per line)" className="w-full bg-[#030303] border border-[#1a1a1a] rounded-lg px-3 py-2 text-sm" />
          <textarea value={(draft.quotes || []).join("\n")} onChange={e => editField("quotes", e.target.value.split("\n").filter(Boolean))}
            rows={3} placeholder="Quotes (one per line)" className="w-full bg-[#030303] border border-[#1a1a1a] rounded-lg px-3 py-2 text-sm" />
        </div>
      )}
    </section>
  )
    ...
    <div className={c.page}>
      ...
      <header id="app-header" className={c.header}>
        <div className={c.headerInner}>
          <div className={c.brand}><span className={c.dot}></span>Pulse</div>
          <ViewerTag />
        </div>
      </header>
