...
function MyTimerPanel({ viewer, database, sessions, ViewerTag }) {
  ...
  async function startPomo(e) {
    ...
    setIsStarting(true)
    try {
      let category = "focus", nudge = "Stay sharp."
      try {
      ...
      await database.put({
        type: "session", status: "active",
        label: label.trim(), category, nudge,
        authorHandle: viewer.userHandle,
        startedAt, endsAt: startedAt + POMO_SECONDS * 1000
      })
      setLabel("")
  ...
  if (mySession) {
    return (
      <section id="my-timer" className="rounded-2xl border border-[color:var(--accent)] bg-[color:var(--card-bg)] p-5">
        ...
        <div className="mono text-5xl font-bold tracking-tight tabular-nums">{mm}:{ss}</div>
        <p className="text-base font-medium mt-2">{mySession.label}</p>
        <div className="flex flex-wrap gap-2 mt-1">
          <span className="mono text-[10px] uppercase tracking-widest text-[color:var(--muted)] border border-[color:var(--border)] px-2 py-1 rounded">{mySession.category}</span>
        </div>
        <p className="text-xs text-[color:var(--muted)] italic mt-3">"{mySession.nudge}"</p>
        <div className="flex gap-2 mt-4">
          <button onClick={() => endSession("done")} className="flex-1 min-h-[44px] rounded-xl bg-[color:var(--accent)] text-[color:var(--bg)] font-semibold">Finish</button>
          <button onClick={() => endSession("cancelled")} className="flex-1 min-h-[44px] rounded-xl border border-[color:var(--border)] text-[color:var(--text)]">Cancel</button>
        </div>
      </section>
    )
  }
    ...
    <section id="my-timer" className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--card-bg)] p-5">
      ...
      <form onSubmit={startPomo} className="space-y-3">
        ...
        <div className="flex gap-2">
          <button type="submit" disabled={!label.trim() || isStarting} className="flex-1 min-h-[44px] rounded-xl bg-[color:var(--accent)] text-[color:var(--bg)] font-semibold disabled:opacity-40 flex items-center justify-center gap-2">
            {isStarting ? <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9" strokeDasharray="40 20"/></svg> : null}
            {isStarting ? "Starting..." : "Start 25:00"}
          </button>
          <button type="button" onClick={async () => {
            try {
              const res = await callAI("Suggest a short concrete focus task for a developer (5-8 words).", { schema: { properties: { task: { type: "string" } } } })
              setLabel(JSON.parse(res).task || "")
            } catch {}
          }} className="min-h-[44px] px-3 rounded-xl border border-[color:var(--border)] text-xs text-[color:var(--muted)]">Suggest</button>
        </div>
      </form>
    </section>
  )
      ...
      {active.length === 0 ? (
        ...
        <ul className="space-y-2 mb-5">
          {active.map(s => {
            ...
            return (
              <li key={s._id} className={`flex items-center gap-3 p-3 rounded-xl border ${isMe ? "border-[color:var(--accent)]" : "border-[color:var(--border)]"} bg-[color:var(--bg)]`}>
                ...
                <div className="mono text-xl tabular-nums">{mm}:{ss}</div>
              </li>
            )
          })}
        </ul>
        ...
        <ul className="space-y-1">
          {recent.map(s => (
            <li key={s._id} className="flex items-center gap-3 py-2 text-sm border-b border-[color:var(--border)] last:border-0">
              ...
              <span className="mono text-[10px] uppercase tracking-widest text-[color:var(--muted)]">{s.status === "done" ? "done" : s.status === "cancelled" ? "stopped" : "expired"}</span>
            </li>
          ))}
        </ul>
      )}
...
export default function App() {
  ...
  return (
    <main id="app" className="min-h-screen bg-[color:var(--bg)] text-[color:var(--text)]">
      <ThemeStyles />
      <header id="app-header" className="sticky top-0 z-10 bg-[color:var(--bg)]/95 backdrop-blur border-b border-[color:var(--border)]">
