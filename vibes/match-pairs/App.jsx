...
function Shape({ name }) {
  const props = { width: "60%", height: "60%", viewBox: "0 0 24 24", fill: "none", stroke: "#7dde4a", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" };
  const shapes = {
    ...
    dot: <circle cx="12" cy="12" r="3" fill="#7dde4a" />,
...
// board. Tap to flip, find the pairs, race your own best. Adapted from the
// hand-tuned jchris/memory-pairs (a mind-games featured app) — reworked to pure
// local state (Bloom-style) so an anonymous /start visitor can PLAY, not
// spectate; personal bests persist per device in localStorage. This is the
...
export default function App() {
  ...
  function flipTile(idx) {
    setGame((g) => {
      ...
      if (tiles[a].shape === tiles[b].shape) {
        ...
        if (completed) saveBest(g.sizeKey, moves, finishedAt - g.startedAt);
  ...
  const c = {
    ...
    header: "sticky top-0 bg-[#1a1a1a] border-b border-white/15 px-4 py-3 flex items-center justify-between",
    title: "text-xl font-bold tracking-tight",
    tagline: "text-xs text-white/50 font-mono",
    main: "px-4 py-4 max-w-2xl mx-auto space-y-4 pb-24",
    section: "bg-[#1a1a1a] border border-white/15 rounded-lg p-4",
    sectionTitle: "text-sm font-mono uppercase tracking-wider text-white/50 mb-3",
    btn: "min-h-[44px] px-4 py-3 bg-[#7dde4a] text-black font-semibold rounded-md active:scale-95 transition disabled:opacity-40",
    ...
    tile: "aspect-square rounded-md bg-[#2d1a4a] border-2 border-[#7040a0] active:scale-95 transition flex items-center justify-center cursor-pointer",
    tileBack: "bg-[#2d1a4a] border-[#7040a0] hover:border-[#7dde4a] hover:bg-[#3a2060]",
    statBox: "flex-1 bg-[#0f0f0f] border border-white/10 rounded-md p-3 text-center",
    statLabel: "text-[10px] font-mono uppercase text-white/40",
    statValue: "text-2xl font-bold text-[#7dde4a] font-mono",
  };
  ...
  return (
    <div className={c.page}>
      <header id="app-header" className={c.header}>
        <div>
          <h1 className={c.title}>Match Pairs</h1>
          <p className={c.tagline}>flip · match · remember</p>
        </div>
      </header>
      ...
      <main id="app" className={c.main}>
        ...
        <section id="stats" className={c.section}>
          ...
          {game.completed && <p className="text-center mt-3 text-[#7dde4a] font-mono text-sm">✦ Complete! ✦</p>}
        </section>
            ...
            {game.tiles.map((t, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => flipTile(i)}
                  disabled={show}
                  className={c.tile + " " + (t.matched ? "opacity-40 bg-[#7dde4a]/10 border-[#7dde4a]/50" : show ? "bg-[#7dde4a]/10 border-[#7dde4a]/50" : c.tileBack)}
                >
                  {show ? <Shape name={t.shape} /> : <span className="text-[#7040a0] text-lg select-none">✦</span>}
                </button>
              );
            })}
          </div>
                ...
                <li key={k} className="flex items-center justify-between py-2 border-b border-white/10 last:border-0">
                  ...
                  {b ? (
                    <span className="text-sm font-mono">
                      <span className="text-[#7dde4a]">{b.moves}</span> moves · {Math.floor(b.elapsed / 1000)}s
                    </span>
                  ) : (
