...
function Header({ ViewerTag, c }) {
  return (
    <header id="app-header" className={`${c.headerBg} sticky top-0 z-10 px-4 py-3 border-b-[length:var(--border-width)] border-[var(--border)] flex items-center justify-between`}>
...
function AddTaskForm({ c, database, useDocument, can, ready }) {
  ...
  if (!ready) return <section className={`${c.surface} p-4 rounded-[var(--radius)] border-[length:var(--border-width)] border-[var(--border)]`}><p className={c.muted}>Loading...</p></section>

  const verdict = can.create({ type: "task", text: doc.text, done: false })
  if (!verdict.ok) {
    return (
      <section id="add-task" className={`${c.surface} p-4 rounded-[var(--radius)] border-[length:var(--border-width)] border-[var(--border)]`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={suggest} disabled={isLoading || !doc.text.trim()} className={c.btnGhost}>
            {isLoading ? (
              <svg className="animate-spin inline" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" strokeDasharray="40 20" /></svg>
            ) : "Suggest"}
          </button>
        </div>
      </form>
...
function TaskList({ c, tasks, database, can }) {
  ...
  return (
    <section id="task-list" className={`${c.surface} p-4 rounded-[var(--radius)] border-[length:var(--border-width)] border-[var(--border)]`}>
      ...
      {tasks.length === 0 ? (
        ...
        <ul className="space-y-2">
          {tasks.map(task => {
            ...
            return (
              <li key={task._id} className={`flex items-center gap-3 p-3 rounded-[var(--radius-sm)] border-[length:var(--border-width)] border-[var(--border)] ${isSaving ? 'opacity-60' : ''}`}>
                <input
                  type="checkbox"
                  checked={task.done}
...
export default function App() {
  ...
  const c = {
    ...
    input: 'bg-[var(--background)] text-[var(--text-primary)] border-[length:var(--border-width)] border-[var(--border)] rounded-[var(--radius-sm)] px-3 py-3 w-full min-h-[44px]',
    btn: 'bg-[var(--primary)] text-[var(--accent-text)] rounded-[var(--radius-sm)] px-4 py-3 min-h-[44px] font-medium disabled:opacity-50',
    btnGhost: 'bg-transparent text-[var(--text-secondary)] rounded-[var(--radius-sm)] px-3 py-2 min-h-[44px] border-[length:var(--border-width)] border-[var(--border)]',
  }
