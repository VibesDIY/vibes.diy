    ...
    try {
      ...
      try {
        const resp = await callAI(
          `Triage this apartment maintenance request. Unit ${unit}. Problem: ${description}`,
          { schema: { properties: {
  ...
  if (!viewer) {
    return (
      <section id="request-form" className="border-2 p-4" style={{ borderColor: 'var(--border)' }}>
        ...
        <p style={{ color: 'var(--muted)' }} className="text-sm">Sign in to submit a maintenance request.</p>
      </section>
    )
  }
...
function MyRequests({ viewer }) {
  ...
  return (
    <section id="my-requests" className="border-2 p-4" style={{ borderColor: 'var(--border)' }}>
      ...
      {sorted.length === 0 ? (
        <p style={{ color: 'var(--muted)' }} className="text-sm">No requests yet. Submit one above.</p>
      ) : (
            ...
            return (
              <li key={r._id} className="border-2 p-3" style={{ borderColor: 'var(--border)' }}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="mono text-xs px-2 py-1 border" style={{ background: sc.bg, color: sc.text, borderColor: 'var(--border)' }}>
...
function ManagerDashboard({ ViewerTag }) {
  ...
  return (
    <section id="manager-dashboard" className="border-2 p-4" style={{ borderColor: 'var(--border)' }}>
      ...
      {visible.length === 0 ? (
        <p style={{ color: 'var(--muted)' }} className="text-sm">No requests in this view.</p>
      ) : (
        <ul className="space-y-3">
          {visible.map(r => {
            const sc = statusColor(r.status)
            return (
              <li key={r._id} className="border-2 p-3" style={{ borderColor: 'var(--border)' }}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    <div className="mono text-xs font-bold">UNIT {r.unit}</div>
                    <div className="text-xs mt-1"><ViewerTag userHandle={r.authorHandle} /></div>
                  </div>
                  <div className="mono text-xs px-2 py-1 border" style={{ background: sc.bg, color: sc.text, borderColor: 'var(--border)' }}>
                    {(r.status || "open").toUpperCase()}
                  </div>
