...
function MilestoneBoard({ database, milestones, updates, viewer, isOwner, ViewerTag }) {
  ...
  async function suggest() {
    ...
    setSuggesting(true)
    try {
      const res = await callAI(`Break this team milestone into 3-5 concrete sub-tasks and success criteria. Milestone: "${title}". ${description ? "Context: " + description : ""}`, {
  ...
  async function create(e) {
    ...
    setBusy(true)
    try {
      await database.put({ type: "milestone", title: title.trim(), description, deadline, subtasks, complete: false, createdAt: Date.now() })
      setTitle(""); setDescription(""); setDeadline(""); setSubtasks([])
    } finally { setBusy(false) }
  }

  const updateCounts = updates.reduce((acc, u) => { if (u.type === "update") acc[u.milestoneId] = (acc[u.milestoneId] || 0) + 1; return acc }, {})
    ...
    <section id="milestones" className="border-2 border-[oklch(0.05_0.01_0)] bg-white/40 p-4 rounded-sm">
      ...
      {isOwner && (
        <form onSubmit={create} className="space-y-2 mb-5 pb-5 border-b-2 border-dashed border-[oklch(0.05_0.01_0)]">
          ...
          <textarea className={ip} placeholder="Description / success criteria" rows={2} value={description} onChange={e => setDescription(e.target.value)} />
          ...
          <div className="flex gap-2">
            <button type="button" onClick={suggest} disabled={suggesting || !title.trim()} className={`${btn} bg-white disabled:opacity-50`}>
              {suggesting ? <svg className="animate-spin w-3 h-3 inline" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeDasharray="40 20"/></svg> : "AI sub-tasks"}
            </button>
            <button type="submit" disabled={busy || !title.trim()} className={`${btn} bg-[oklch(0.90_0.20_110)] disabled:opacity-50 flex-1`}>
      ...
      <ul className="space-y-3">
        {milestones.map(m => (
          <li key={m._id} className={`border-2 border-[oklch(0.05_0.01_0)] p-3 ${m.complete ? "bg-[oklch(0.90_0.20_110)]/30" : "bg-white"}`}>
              ...
              <div className="flex-1">
                ...
                {m.subtasks?.length > 0 && (
                  <ul className="text-xs font-mono mt-2 space-y-1 text-[oklch(0.40_0.01_0)]">
                    {m.subtasks.map((s, i) => <li key={i}>▸ {s}</li>)}
                  </ul>
                )}
                <div className="text-xs font-mono mt-2 text-[oklch(0.40_0.01_0)]">{updateCounts[m._id] || 0} update{updateCounts[m._id] === 1 ? "" : "s"}</div>
              </div>
              {isOwner && (
                <button onClick={() => database.put({ ...m, complete: !m.complete })} className={`${btn} ${m.complete ? "bg-white" : "bg-[oklch(0.90_0.20_110)]"}`}>
...
function UpdateForm({ milestoneId, database, viewer }) {
  ...
  async function post(e) {
    ...
    setBusy(true)
    try {
      await database.put({ type: "update", milestoneId, body: body.trim(), authorHandle: viewer.userHandle, createdAt: Date.now() })
      setBody("")
    } finally { setBusy(false) }
  }
  return (
    <form onSubmit={post} className="flex gap-2 mt-3">
      <input className="flex-1 border-2 border-[oklch(0.05_0.01_0)] bg-white px-2 py-2 text-sm font-mono min-h-[40px]" placeholder="Post an update…" value={body} onChange={e => setBody(e.target.value)} />
      <button type="submit" disabled={busy || !body.trim()} className="px-3 py-2 border-2 border-[oklch(0.05_0.01_0)] bg-[oklch(0.90_0.20_110)] font-mono text-xs uppercase tracking-widest disabled:opacity-50">Post</button>
    </form>
  )
}
  ...
  return (
    <div className={c.page}>
      ...
      <main id="app" className={c.main}>
        <MilestoneBoard database={database} milestones={milestones} updates={updates} viewer={viewer} isOwner={isOwner} ViewerTag={ViewerTag} />
