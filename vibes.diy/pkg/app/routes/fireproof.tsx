...
export default function FireproofRoute() {
  ...
  const tenantsQuery = useQuery({
    ...
    queryFn: async () => {
        ...
        if (res.isErr()) {
            ...
            throw new Error(res.Err().message || res.Err() || "Failed to list tenants");
        }
        return res.Ok();
    },
  ...
  const ledgersQuery = useQuery({
    ...
    queryFn: async () => {
        const res = await api.listLedgersByUser({});
         if (res.isErr()) {
            // @ts-ignore
            throw new Error(res.Err().message || res.Err() || "Failed to list ledgers");
        }
        return res.Ok();
    },
    enabled: isLoaded && isSignedIn && !!session,
  ...
  if (!isLoaded) return <SimpleAppLayout><div>Loading session...</div></SimpleAppLayout>;
  if (!isSignedIn) return <SimpleAppLayout><div>Please sign in to view Fireproof data.</div></SimpleAppLayout>;
      ...
      <div className="container mx-auto p-4 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="border p-4 rounded shadow-sm">
            ...
            {tenantsQuery.isError && <div className="text-red-500">Error: {tenantsQuery.error.message}</div>}
            {tenantsQuery.data && (
                ...
                {tenantsQuery.data.tenants.length === 0 ? (
                  ...
                  tenantsQuery.data.tenants.map((t) => (
                    <li key={t.tenantId} className="border-b pb-2">
                       <div className="font-medium">{t.tenant.name || "Unnamed Tenant"}</div>
                       <div className="text-xs text-gray-500">ID: {t.tenantId}</div>
                       <div className="text-xs text-gray-500">Role: {t.role}</div>
                    </li>
                  ))
                )}
              </ul>
          ...
          <div className="border p-4 rounded shadow-sm">
            <h2 className="text-xl font-semibold mb-4">Ledgers</h2>
             ...
             {ledgersQuery.isError && <div className="text-red-500">Error: {ledgersQuery.error.message}</div>}
             {ledgersQuery.data && (
                <ul className="space-y-2">
                 {ledgersQuery.data.ledgers.length === 0 ? (
                   ...
                   ledgersQuery.data.ledgers.map((l) => (
                     <li key={l.ledgerId} className="border-b pb-2">
                       <div className="font-medium">{l.name || "Unnamed Ledger"}</div>
                       <div className="text-xs text-gray-500">ID: {l.ledgerId}</div>
                       <div className="text-xs text-gray-500">Tenant ID: {l.tenantId}</div>
                     </li>
                   ))
                 )}
               </ul>
