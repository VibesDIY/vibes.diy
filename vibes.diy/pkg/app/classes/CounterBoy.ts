...
import { EXPLOSION, ANIMATION_DURATIONS, SCENE_DIMENSIONS, COLORS } from '../constants/scene'
import {
  makeEnclosure,
  makeEnclosureBack,
  makeChrome,
...
export class CounterBoy {
  ...
  private createObjects() {
    ...
    this.originalEnclosureGeometry = this.enclosure.geometry as THREE.BoxGeometry
  ...
  public animateBlockEncryption(
    ...
    const visibleEncryptedBlocks = this.blockSituations.filter(situation =>
      (situation.encryptedBlock as any).visible
    )
    ...
    if (newBlockSituation) {
      ...
      if (skipUnencrypted) {
        ...
        ;(newBlockSituation.group as any).remove(newBlockSituation.unencryptedBlock)
  ...
  private createNewBlockSituation(preset: { hexPair: string; textureOffsetX: number; textureOffsetY: number }): BlockSituationObjects | null {
  ...
  private animateNewBlock(blockSituation: BlockSituationObjects, onComplete?: () => void) {
    ...
    this.blockAnimationRef = animate(unencryptedBlock.position, {
      ...
      onRender: () => {
        ...
        if (unencryptedBlock.position.z <= 1 && !(encryptedBlock as any).visible) {
  ...
  private animateEnclosureThickness(targetThickness: number, duration: number = 400, onComplete?: () => void) {
    // @ts-ignore - geometry exists on Mesh
  ...
  public setExplosionProgress(progress: number) {
    ...
    if (progress <= 0.5) {
      ...
      enclosureBackZ = lerp(EXPLOSION.TENSION_POSITIONS.ENCLOSURE_BACK_Z, EXPLOSION.POSITIONS.ENCLOSURE_BACK_Z * EXPLOSION.FACTOR, t)
  ...
  private updateDisplayTexture() {
    ...
    // right
    ctx.textAlign = 'right'
    ctx.fillText(this.encryptedBlocksCount.toString(), SCENE_DIMENSIONS.DISPLAY.TEXT_X, (canvas.height / 2) + (SCENE_DIMENSIONS.DISPLAY.FONT_SIZE / 2))
