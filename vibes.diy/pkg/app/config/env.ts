...
// class vibesDiyEnv {
//   readonly env = Lazy(() => {
//     const out = callAiEnv.merge(ensureSuperThis().env);
//     for (const k in import.meta.env) {
//       out.set(k, (import.meta.env as Record<string, string>)[k]);
//       out.set(k.replace("VITE_", ""), (import.meta.env as Record<string, string>)[k]);
//     }
//     // console.log("env", out.keys())
//     return out;
//   });

//   readonly PROMPT_FALL_BACKURL = Lazy(
//     () => new URL(this.env().get("PROMPT_FALL_BACKURL") ?? "https://esm.sh/@vibes.diy/prompts/llms")
//   );

//   readonly GA_TRACKING_ID = Lazy(() => this.env().get("VITE_GOOGLE_ANALYTICS_ID") ?? this.env().get("GOOGLE_ANALYTICS_ID") ?? "");

//   // Google Tag Manager
//   readonly GTM_CONTAINER_ID = Lazy(() => this.env().get("VITE_GTM_CONTAINER_ID") ?? this.env().get("GTM_CONTAINER_ID") ?? "");

//   // PostHog
//   readonly POSTHOG_KEY = Lazy(() => this.env().get("VITE_POSTHOG_KEY") ?? this.env().get("POSTHOG_KEY") ?? "");
//   readonly POSTHOG_HOST = Lazy(() => this.env().get("VITE_POSTHOG_HOST") ?? this.env().get("POSTHOG_HOST") ?? "");

//   // Application Behavior
//   readonly APP_MODE = Lazy(() => this.env().get("MODE") ?? "production");
//   readonly APP_BASENAME = Lazy(() => this.env().get("VITE_APP_BASENAME") ?? this.env().get("APP_BASENAME") ?? "/");
...
//     () => new URL(this.env().get("VITE_APP_HOST_BASE_URL") ?? this.env().get("APP_HOST_BASE_URL") ?? "https://vibesdiy.app").href // Keep trailing slash - standardize on YES trailing slash
//   );
...
//       return envKey;
//     }
//     throw new Error("CLERK_PUBLISHABLE_KEY is required in environment variables");
...
//     const envUrl = this.env().get("VITE_API_BASE_URL") ?? this.env().get("API_BASE_URL");
//     if (envUrl) {
//       return new URL(envUrl).href;
//     }
//     // Use production worker ONLY for vibes.diy, preview worker for everything else
//     const isProduction = runtimeFn().isBrowser && window.location.hostname.endsWith("vibes.diy");
//     const defaultUrl = isProduction ? "https://vibes-diy-api.com" : "https://vibes-hosting-v2-preview.jchris.workers.dev";
...
//     () => new URL(this.env().get("VITE_CALLAI_ENDPOINT") ?? this.env().get("CALLAI_ENDPOINT") ?? this.API_BASE_URL()).href // Keep trailing slash - standardize on YES trailing slash
//   );

//   // Chat History Database
//   readonly SETTINGS_DBNAME = Lazy(
//     () => this.env().get("VITE_VIBES_CHAT_HISTORY") ?? this.env().get("VIBES_CHAT_HISTORY") ?? "vibes-chats"
//   );
...
//       WORKSPACE_NPM_URL: workspace_npm_url,
//       PUBLIC_NPM_URL: public_npm_url,
//     };
//     console.log(`Configuring VibesEnv with`, ret);
//     return ret;
//   });
// }
