 ...
 */
export async function generateTitle(_segments: Segment[], _model: string, _apiKey = ""): Promise<string> {
  ...
  try {
    ...
    const title = "Title-Unkown"; // (await callAI(messages, options)) as string;
