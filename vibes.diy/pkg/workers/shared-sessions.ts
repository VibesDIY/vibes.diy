...
function userNotifyCallbacksForSharedSessions(shard: string, env: CFEnv) {
  ...
  // SHARED_SESSIONS rather than CHAT_SESSIONS. (#2265 Track B)
  ...
  return {
    registerUserSubscription: async (userId: string): Promise<void> => {
      if (shard !== userNotifyShardFor(userId)) {
        console.warn("[SharedSessions] skip user-notify register: shard does not match authenticated user", shard.slice(0, 16));
        return;
      }
...
export class SharedSessions implements DurableObject {
  ...
  async fetch(request: CFRequest): Promise<CFResponse> {
    ...
    // SharedSessions never evaluates access fns — no invokeAccessFn needed.
