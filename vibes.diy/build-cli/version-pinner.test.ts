...
describe("VersionPinner", () => {
  ...
  beforeAll(async () => {
    ...
    findUpVersion = await pnpmService.getInstalledVersion("find-up");
  });
  ...
  describe("pinVersions", () => {
    ...
    it("should pin unpinned dependencies with asterisk (*)", () => {
      ...
      expect(result.dependencies["find-up"]).toBe(findUpVersion);
    });
    ...
    it("should sort dependencies alphabetically", async () => {
      ...
      expect(keys).toEqual(["cmd-ts", "find-up", "semver", "zx"]);
    });
    ...
    it("should handle mixed pinned and unpinned dependencies", () => {
      const pkg: PackageJson = {
        ...
        dependencies: {
          ...
          "find-up": `~${findUpVersion}`, // needs pinning
        },
      };
      ...
      expect(result.dependencies["find-up"]).toBe(`~${findUpVersion}`); // pinned
    });
