...
describe("runtime/preview iframe policy", () => {
  it("includes autoplay/encrypted-media while preserving camera/microphone", () => {
    ...
    expect(allowTokens).toEqual(["autoplay", "camera", "clipboard-write", "encrypted-media", "microphone"]);
  ...
  it("retains popup sandbox delegation tokens", () => {
    ...
    expect(sandboxTokens).toEqual(expect.arrayContaining(["allow-popups", "allow-popups-to-escape-sandbox"]));
