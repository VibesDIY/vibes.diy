...
import { notifyCommentPosted } from "@vibes.diy/api-svc";
...
export const evtCommentPostedEvento: EventoHandler<unknown, MsgBase<EvtCommentPosted>, void> = {
  ...
  handle: async (ctx: HandleTriggerCtx<unknown, MsgBase<EvtCommentPosted>, void>): Promise<Result<EventoResultType>> => {
    ...
    // Persist a comment-posted notification for the owner (and fan out the live
    // bell). Dedupe is per-comment (docId), so re-delivery does not double-notify.
