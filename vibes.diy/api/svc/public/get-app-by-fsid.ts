    ...
    async (
      ...
      if (isOwner) {
        ...
        const rAppSet = await ensureAppSettings(vctx, {
          ...
          auth: { type: "not-loggedin", token: req.token ?? "" },
          appSlug: app.appSlug,
        ...
        if (settings.entry.enableRequest) {
          if (!approvetedUserId) {
        ...
        if (!grant && settings.entry.publicAccess && app.mode === "production") {
