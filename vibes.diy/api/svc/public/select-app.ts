...
// Resolve the owner's latest app row for (ownerHandle, appSlug) across ALL modes
// (#2772). Unlike selectLatestAppPerSlug (which prefers production), this returns the
// single most-recently-created row, so a newer in-place `dev` draft wins — "even a
// draft auto-resolves its latest fsId the way production does" (the #2772 principle).
// Falls back to the latest production when no dev exists, and after a publish the new
// production row is newest-created → this returns it, so the draft indicator clears.
//
// OWNER-ONLY by contract: the caller (get-app-by-fsid) verifies the selected row's
// userId matches the authenticated caller before honoring it, and otherwise serves
// the published path. On a created-timestamp tie, mode DESC makes "production" win so
// a stale tie never raises a spurious draft.
export async function selectLatestDraftOrPublished(
  vctx: VibesApiSQLCtx,
  req: { readonly ownerHandle: string; readonly appSlug: string }
) {
  const rows = await vctx.sql.db
    ...
    .from(vctx.sql.tables.apps)
    .where(and(eq(vctx.sql.tables.apps.ownerHandle, req.ownerHandle), eq(vctx.sql.tables.apps.appSlug, req.appSlug)))
    .orderBy(desc(vctx.sql.tables.apps.created), desc(vctx.sql.tables.apps.mode))
    .limit(1);
  return rows[0];
}
