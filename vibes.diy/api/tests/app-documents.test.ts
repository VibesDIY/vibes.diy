...
describe("Firefly app-documents", { timeout: 10000 }, () => {
  ...
  it("putDoc creates a document and returns id", async () => {
    const rRes = await api.putDoc({ appSlug, doc: { title: "hello" } });
  ...
  it("putDoc with explicit docId uses that id", async () => {
    const rRes = await api.putDoc({ appSlug, doc: { title: "explicit" }, docId: "my-doc-id" });
    expect(rRes.isOk()).toBe(true);
    expect(rRes.Ok().id).toBe("my-doc-id");
  });

  it("getDoc retrieves latest revision", async () => {
    const putRes = await api.putDoc({ appSlug, doc: { title: "getme" }, docId: "get-test" });
    expect(putRes.isOk()).toBe(true);

    const rRes = await api.getDoc({ appSlug, docId: "get-test" });
    expect(rRes.isOk()).toBe(true);
    const res = rRes.Ok();
    expect(res.status).toBe("ok");
  ...
  it("putDoc same docId increments seq, latest wins", async () => {
    await api.putDoc({ appSlug, doc: { title: "v1" }, docId: "seq-test" });
    await api.putDoc({ appSlug, doc: { title: "v2" }, docId: "seq-test" });

    const rRes = await api.getDoc({ appSlug, docId: "seq-test" });
    expect(rRes.isOk()).toBe(true);
    const res = rRes.Ok();
  ...
  it("getDoc returns not-found for missing doc", async () => {
    const rRes = await api.getDoc({ appSlug, docId: "nonexistent" });
    expect(rRes.isOk()).toBe(true);
    expect(rRes.Ok().status).toBe("not-found");
  });

  it("deleteDoc inserts tombstone", async () => {
    await api.putDoc({ appSlug, doc: { title: "delete-me" }, docId: "del-test" });
    const rRes = await api.deleteDoc({ appSlug, docId: "del-test" });
    expect(rRes.isOk()).toBe(true);
    expect(rRes.Ok().status).toBe("ok");
  });

  it("getDoc returns not-found for deleted doc", async () => {
    await api.putDoc({ appSlug, doc: { title: "soon-gone" }, docId: "del-get-test" });
    await api.deleteDoc({ appSlug, docId: "del-get-test" });

    const rRes = await api.getDoc({ appSlug, docId: "del-get-test" });
    expect(rRes.isOk()).toBe(true);
    expect(rRes.Ok().status).toBe("not-found");
  });
  ...
  it("queryDocs returns all non-deleted docs", async () => {
    ...
    await api.putDoc({ appSlug, doc: { title: "one" }, docId: `${prefix}-1` });
    await api.putDoc({ appSlug, doc: { title: "two" }, docId: `${prefix}-2` });
    await api.putDoc({ appSlug, doc: { title: "three" }, docId: `${prefix}-3` });

    const rRes = await api.queryDocs({ appSlug });
    expect(rRes.isOk()).toBe(true);
  ...
  it("queryDocs deduplicates by latest seq", async () => {
    ...
    await api.putDoc({ appSlug, doc: { title: "v1" }, docId });
    await api.putDoc({ appSlug, doc: { title: "v2" }, docId });
  ...
  it("queryDocs excludes deleted docs", async () => {
    ...
    await api.putDoc({ appSlug, doc: { title: "gone" }, docId });
    await api.deleteDoc({ appSlug, docId });
  ...
  it("subscribeDocs returns ok", async () => {
    const rRes = await api.subscribeDocs({ appSlug });
    expect(rRes.isOk()).toBe(true);
    expect(rRes.Ok().status).toBe("ok");
  });
});
