...
export function HiddenMenuWrapper({ children, menuContent, triggerBounce, showVibesSwitch = true }: HiddenMenuWrapperProps) {
  ...
  return (
    <div style={HiddenMenuWrapperStyles.getWrapperStyle()}>
      {/* Menu */}
      <div
        id="hidden-menu"
        role="dialog"
        ...
        ref={menuRef}
        style={HiddenMenuWrapperStyles.getMenuStyle()}
      >
        {menuContent}
      </div>

      {/* Content */}
      <div style={HiddenMenuWrapperStyles.getContentWrapperStyle(menuHeight, menuOpen, isBouncing)} ref={menuContainerRef}>
        <div style={HiddenMenuWrapperStyles.getInnerContentWrapperStyle(menuOpen)}>
          <div style={HiddenMenuWrapperStyles.getContentStyle()}>{children}</div>
        </div>
      </div>
      ...
      {showVibesSwitch && (
        <button
          ...
          style={HiddenMenuWrapperStyles.getToggleButtonStyle()}
        >
          <VibesSwitch size={80} />
        </button>
      )}
