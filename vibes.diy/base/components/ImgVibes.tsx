...
import type { ImgVibesOptions } from "call-ai";
import { useImgVibes as defaultUseImgVibes } from "../hooks/img-vibes/use-img-vibes.js";
import { useFireproof, Database } from "@fireproof/use-fireproof";
import { ensureSuperThis } from "@fireproof/core-runtime";
import { ImgVibesPromptWaiting, ImgVibesDisplayPlaceholder, ImgVibesDisplay, ImgVibesError } from "./ImgVibesUtils.js";
// Import from direct file since the main index.ts might not be updated yet
import { ImgVibesUploadWaiting } from "./ImgVibesUtils/ImgVibesUploadWaiting.js";
import { getImgVibesMode } from "./ImgVibesUtils/ImgVibesModeUtils.js";
import { defaultClasses } from "../utils/style-utils.js";
import { logDebug } from "../utils/debug.js";
import { ImageDocument, ImgVibesClasses, UseImgVibesOptions, UseImgVibesResult } from "@vibes.diy/use-vibes-types";
...
/**
 * Core implementation of ImgVibes component
 * This is the component that gets remounted when the document ID or prompt changes
 */
function ImgVibesCore(props: Partial<ImgVibesProps>): React.ReactElement {
  ...
  const [imgVibesDocument, setImageGenDocument] = React.useState<ImageDocument | null>(null);
  ...
  const { imageData, loading, error, progress, document } = (useImgVibes || defaultUseImgVibes)({
    ...
    options: {
      ...
      ...(imgVibesDocument ? { document: imgVibesDocument } : {}),
    },
    database,
  ...
  if (debug) {
    logDebug("[ImgVibesCore] Current mode:", mode, {
      document: !!document,
    ...
    async (id: string, newPrompt: string) => {
      ...
      try {
        ...
        if (refreshedDoc) {
          ...
          if (debug) {
            logDebug(
              "[ImgVibes] Setting document for image generation:",
              refreshedDoc._id,
              "with files:",
  ...
  const handleDelete = React.useCallback(
    async (id: string) => {
      logDebug("[ImgVibes] Attempting to delete document:", id);
      try {
        // Use await to ensure the operation completes
        const result = await db.del(id);

        if (debug) {
          logDebug("[ImgVibes] Document deletion result:", result);
        }

        // Notify parent component about deletion
        if (onDelete) {
          if (debug) {
            logDebug("[ImgVibes] Calling onDelete callback with id:", id);
          }
          onDelete(id);
        }
      } catch (error) {
  ...
  const handleDocCreated = React.useCallback(
    (docId: string) => {
      if (debug) {
        logDebug("[ImgVibesCore] Document created:", docId);
      }
  ...
  function renderContent() {
    if (debug) {
      logDebug("[ImgVibes Debug] Render state:", {
        mode,
    ...
    switch (mode) {
      case "placeholder": {
        ...
        // Use the same ImgVibesUploadWaiting component that's used in uploadWaiting mode
        // but without a document (this creates a consistent UI for both entry points)
        return (
          <ImgVibesUploadWaiting
            className={className}
            classes={classes}
            debug={debug}
            database={database}
            ...
            onPromptSubmit={(newPrompt: string) => {
              ...
              if (debug) {
                logDebug("[ImgVibesCore] Prompt submitted from initial view:", newPrompt);
              }
      ...
      case "uploadWaiting": {
        ...
        if (!document || !document._id) {
          ...
          return <ImgVibesPromptWaiting className={className} classes={classes} />;
        }
          ...
          <>
            <ImgVibesUploadWaiting
              ...
              onFilesAdded={() => {
                ...
                if (debug) {
                  logDebug("[ImgVibesCore] Files added to existing document:", document._id);
                }
              }}
              onPromptSubmit={(newPrompt: string, docId?: string) => {
                ...
                if (debug) {
                  logDebug("[ImgVibesCore] Prompt submitted for existing uploads:", newPrompt);
                  logDebug("[ImgVibesCore] Using document ID:", targetDocId);
                }
      ...
      case "generating": {
        ...
        if (debug) {
          logDebug("[ImgVibes Debug] Generating state prompt sources:", {
            currentEditedPrompt: currentEditedPrompt || null,
      ...
      case "display": {
        ...
        if (!document || !document._id) {
          return <ImgVibesError message="Missing document" />;
        }
        ...
        return (
          <>
            ...
            {/* ImgVibesDisplay handles the progress bar internally */}
          </>
        );
      }
      ...
      case "error": {
        // Error state
        return <ImgVibesError message={error ? error.message : "Unknown error"} className={className} />;
      }

      default: {
        // Fallback for any unexpected state
        return <ImgVibesError message="Unknown state" />;
      }
    }
  }
...
/**
 * Main component for generating images with call-ai's imgVibes
 ...
 */
export function ImgVibes(props: Partial<ImgVibesProps>): React.ReactElement {
  // Destructure key props for identity-change tracking
  // classes prop is used via the props spread to ImgVibesCore
  ...
  const handleDocCreated = React.useCallback(
    (docId: string) => {
      if (debug) logDebug("[ImgVibes] Document created:", docId);
      ...
      // Call user's callback if provided
      if (onDocumentCreated) {
        if (debug) logDebug("[ImgVibes] Calling onDocumentCreated callback");
        onDocumentCreated(docId);
      }
    },
  ...
  React.useEffect(() => {
    ...
    if (idChanged || (!_id && promptChanged) || uploadedDocIdChanged) {
      if (debug) {
        logDebug("[ImgVibes] Identity change detected, generating new mountKey:", {
          idChanged,
          _id,
      ...
      setMountKey(sthis().nextId().str); // Force a remount of ImgVibesCore
    }
  ...
  if (uploadedDocId && !_id) {
    // Always pass the uploadedDocId to ImgVibesCore so it can access the document
  ...
  return <ImgVibesCore {...coreProps} key={mountKey} />;
}
