...
import { ImgVibesFileDrop } from "./ImgVibesFileDrop.js";
import { useFireproof } from "@fireproof/use-fireproof";

interface ImgVibesPromptWaitingProps {
...
export function ImgVibesPromptWaiting({
  className,
  classes = defaultClasses,
  database,
  debug,
  onFilesUploaded,
}: ImgVibesPromptWaitingProps) {
  ...
  const handleFilesDropped = React.useCallback(
    async (files: File[]) => {
      ...
      if (debug) {
        console.log(`[ImgVibesPromptWaiting] Handling ${files.length} dropped files`);
      }
      ...
      try {
        ...
        if (debug) {
          console.log("[ImgVibesPromptWaiting] Created document for uploads:", result.id);
        }
        ...
        console.error("[ImgVibesPromptWaiting] Error handling file upload:", error);
      }
    },
