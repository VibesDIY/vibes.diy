...
import { ImgVibesError } from "./ImgVibesError.js";
import { ImgVibesDisplayProps } from "./types.js";
import { combineClasses, defaultClasses } from "../../utils/style-utils.js";
import { getCurrentFileKey, getPromptInfo, getVersionInfo } from "./ImgVibesDisplayUtils.js";
import { ImgVibesModal } from "./ImgVibesModal.js";
import { logDebug } from "../../utils/debug.js";

// Component for displaying the generated image
export function ImgVibesDisplay({
  document,
  className,
  alt,
  onDelete,
  ...
  // ESC handling moved to ImgVibesModal component
  ...
  function handleDeleteConfirm() {
    if (debug) {
      logDebug("[ImgVibesDisplay] handleDeleteConfirm called, document ID:", document?._id);
    }

    if (onDelete && document && document._id) {
      if (debug) {
        logDebug("[ImgVibesDisplay] Calling onDelete with ID:", document._id);
      }
      onDelete(document._id);
    } else {
      console.error("[ImgVibesDisplay] Cannot delete - missing onDelete handler or document ID");
      if (debug) {
        logDebug("[ImgVibesDisplay] Delete details:", {
  ...
  if (debug) {
    logDebug("[ImgVibesDisplay Debug] Render state:", {
      documentId: document?._id,
  ...
  if (!document?._files || (!fileKey && !document._files.image)) {
    if (debug) {
      logDebug("[ImgVibesDisplay Debug] Missing image file - showing error", {
        hasFiles: !!document?._files,
        fileKey,
    ...
    return <ImgVibesError message="Missing image file" />;
  }
  ...
  return (
    <div className={combineClasses("imggen-root", className, classes.root)} title={displayPrompt}>
      ...
      {/* Use the new ImgVibesModal component */}
      <ImgVibesModal
        isOpen={isFullscreen}
