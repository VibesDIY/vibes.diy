...
interface ImgVibesFileDropProps {
 ...
 */
export function ImgVibesFileDrop({
  onFilesDropped,
  className,
  isActive = true,
  maxFiles = 10,
...
}: ImgVibesFileDropProps): React.ReactElement {
  ...
  const processFiles = React.useCallback(
    (fileList: FileList) => {
      ...
      if (debug) {
        console.log(`[ImgVibesFileDrop] Processing ${filesToProcess.length} files of ${files.length} selected`);
      }
  ...
  return (
    <div
      ...
      style={{
        ...imgVibesStyles.fileDrop,
        ...(isDragging ? imgVibesStyles.fileDropActive : {}),
        ...(!isActive ? imgVibesStyles.fileDropDisabled : {}),
      }}
    ...
    >
      ...
      <div className="imggen-file-drop-message" style={imgVibesStyles.fileDropMessage}>
        {addFilesMessage}
      </div>
    </div>
  );
