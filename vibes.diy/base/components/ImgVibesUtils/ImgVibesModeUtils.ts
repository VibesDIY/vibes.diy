...
/**
 * Possible display modes for the ImgVibes component
 */
export type ImgVibesMode =
...
/**
 * Pure function to determine the current mode of the ImgVibes component
 * based on available document data and component state
 */
export function getImgVibesMode({
  document,
  prompt,
  loading,
  error,
  ...
  if (error) {
    if (debug) console.log("[ImgVibesModeUtils] Error present - error mode");
    return "error";
  }
  ...
  if (loading && (prompt || effectivePrompt) && !hasVersions) {
    if (debug) console.log("[ImgVibesModeUtils] Prompt + loading + no versions → generating");
    return "generating";
  }
  ...
  if (hasVersions) {
    if (debug) console.log("[ImgVibesModeUtils] Has versions - display mode");
    return "display";
  }
  ...
  if (debug) {
    console.log("[ImgVibesModeUtils] Determining mode:", {
      prompt: !!prompt,
  ...
  if (!document) {
    if (debug) console.log("[ImgVibesModeUtils] No document - placeholder mode");
    return "placeholder";
  }

  // Case 3: Has input files but no prompt yet - stay in upload waiting mode
  if (hasInputFiles && !effectivePrompt && !hasVersions) {
    if (debug) console.log("[ImgVibesModeUtils] Has input files but no effective prompt - uploadWaiting mode");
  ...
  if (document && effectivePrompt && !hasVersions) {
    if (debug) console.log("[ImgVibesModeUtils] Has document + prompt but no versions - generating mode");
    return "generating";
  }

  // Fallback - if we have an empty document or other invalid state, go back to placeholder
  if (debug) console.log("[ImgVibesModeUtils] Fallback - placeholder mode");
  return "placeholder";
}
