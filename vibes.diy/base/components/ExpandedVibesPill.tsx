...
const vibesD = `M165.866,285.985c-7.999-0.416-19.597-0.733-31.141-1.687  c-15.692-1.297-28.809-8.481-40.105-19.104c-12.77-12.008-20.478-26.828-22.714-44.177c-3.048-23.644,3.384-44.558,19.646-62.143  c9.174-9.92,20.248-17.25,33.444-20.363c7.786-1.837,15.944-2.399,23.973-2.828c9.988-0.535,111.023-0.666,121.021-0.371  c10.191,0.301,20.433,0.806,30.521,2.175c12.493,1.696,23.132,7.919,32.552,16.091c14.221,12.337,22.777,27.953,25.184,46.594  c2.822,21.859-2.605,41.617-16.777,58.695c-9.494,11.441-21.349,19.648-35.722,23.502c-6.656,1.785-13.724,2.278-20.647,2.77  C276.914,285.721,177.682,285.667,165.866,285.985z`;
...
const vibesLetters = [
  ...
  { delay: "1.3s", d: "M164.636,206.263c0-6.691,0.054-13.383-0.036-20.073  c-0.024-1.851,0.716-2.67,2.449-2.81c0.274-0.022,0.549-0.054,0.823-0.076c5.488-0.445,6.091,0.105,6.091,5.562  c0,12.348,0,24.695,0,37.043c0,2.887-0.354,3.405-3.222,3.618c-1.628,0.121-3.338-0.001-4.91-0.408  c-0.593-0.153-1.265-1.408-1.278-2.171c-0.096-5.584-0.034-11.172-0.022-16.759c0.002-1.308,0-2.617,0-3.926  C164.566,206.263,164.601,206.263,164.636,206.263z" },
];
...
export function ExpandedVibesPill({ size = 75, className, onRemix }: ExpandedVibesPillProps) {
  // Phases: idle → slid → grown → revealed (open)
  //         revealed → hiding → shrinking → idle (close)
  const [phase, setPhase] = useState<"idle" | "slid" | "grown" | "revealed" | "hiding" | "shrinking">("idle");

  const handleClick = () => {
    if (phase === "idle") setPhase("slid");
    else if (phase === "revealed") setPhase("hiding");
  };

  React.useEffect(() => {
    let t: ReturnType<typeof setTimeout>;
    if (phase === "slid") t = setTimeout(() => setPhase("grown"), 400);
    else if (phase === "grown") t = setTimeout(() => setPhase("revealed"), 600);
    else if (phase === "hiding") t = setTimeout(() => setPhase("shrinking"), 400);
    else if (phase === "shrinking") t = setTimeout(() => setPhase("idle"), 500);
    return () => clearTimeout(t);
  }, [phase]);

  const slid = phase !== "idle";
  const grown = phase === "grown" || phase === "revealed" || phase === "hiding";
  const showButtons = phase === "revealed" || phase === "hiding";
  const buttonsVisible = phase === "revealed";
  const active = !slid;

  const diyShift = grown ? 800 : 0;
  const numericSize = typeof size === "number" ? size : parseFloat(size as string) || 75;
  const viewW = 600 + diyShift;
  const height = numericSize;
  const width = (height / 300) * viewW;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {grown && (
          <>
            <rect x="300" y="123" width={diyShift} height="175" fill="#000" style={{ transition: "width 0.5s ease" }} />
            <path fillRule="evenodd" clipRule="evenodd" fill="#000" d={outerPath} style={{ transition: "transform 0.5s ease", transform: `translateX(${diyShift}px)` }} />
          </>
        )}
        ...
        <path
          ...
          fillRule="evenodd"
          clipRule="evenodd"
          d={active ? diyD : vibesD}
          style={{ transition: "d 0.4s ease, transform 0.4s ease", transform: active ? "none" : "translateX(3px)" }}
        />

        {vibesLetters.map((l, i) => (
          <path key={`v${i}`} fillRule="evenodd" clipRule="evenodd" style={{ transition: `fill ${l.delay} ease`, fill: active ? switchColors.secondary : switchColors.primary }} d={l.d} />
        ))}

        <g style={{ transition: "transform 0.5s ease", transform: `translateX(${diyShift}px)` }}>
          {diyLetters.map((l, i) => (
            <path key={`d${i}`} fillRule="evenodd" clipRule="evenodd" style={{ transition: "fill 1s ease", fill: active ? switchColors.primary : switchColors.secondary }} d={l.d} />
          ))}
        </g>
      </svg>
