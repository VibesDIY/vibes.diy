...
export function getImgGenMode({
  ...
  if (hasInputFiles && !effectivePrompt && !hasVersions) {
    if (debug) console.log("[ImgGenModeUtils] Has input files but no effective prompt - uploadWaiting mode");
  ...
  if (document && effectivePrompt && !hasVersions) {
    if (debug) console.log("[ImgGenModeUtils] Has document + prompt but no versions - generating mode");
    return "generating";
  }
