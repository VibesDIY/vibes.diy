...
**Match the shape to the model — and watch for the Form-A trap.** Most apps people ask for are multiplayer even when they sound solo: a todo list, a habit tracker, a journal, a notes app, a workout log, a budget tracker — each visitor keeps _their own_ (per-visitor private: an `authorHandle` check routing each user's docs to a per-user channel `user:<handle>`). A shared board, wall, guestbook, or map is author-owned writes + public read. **Gating the core write of any of these on `ctx.requireRole("owner")` is the Form-A trap: it works for you — you're the owner — and is silently dead for every visitor, who can't do the one thing the app is for, and you never witness the failure.** Reserve `requireRole("owner")` for an app that is genuinely one publisher's to write (a blog, an announcements feed) — never for a tracker, list, board, log, or anything a stranger is meant to use.

**"Invite", "join", "collaborate", "share with", "together", "with my partner/team" → per-object collaboration** — each shared thing is its own space its members reach directly. Use the per-object recipe: a channel per object (`list:<id>`), the creator grants themselves at creation, child docs gate on `ctx.requireAccess("list:<id>")`, and a member-authored `share` doc grants a peer into the same channel (plus a `request` doc — which takes **no** `requireAccess`, so any signed-in user can ask to join — for people who aren't members yet). In a shared object, **any member edits any child doc** — gate child docs on `ctx.requireAccess(objectChannel)`, never on the editor's `authorHandle` (an author check on a shared item locks every collaborator out of each other's items, so a partner can't even check one off). Keep the _object's own_ author/creator field write-once (`if (oldDoc && doc.author !== oldDoc.author) throw`) and a child's object-id immutable.
