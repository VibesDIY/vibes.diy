...
**Most apps are multiplayer even when they sound solo — match the shape to the model.** A todo list, a habit tracker, a journal, a notes app, a workout log, or a budget gives each visitor _their own private_ space — not a shared feed: route every doc to a per-user channel named `user:` + the signed-in handle AND **self-grant that same channel** to that user (`grant: { users: { [user.userHandle]: ['user:' + user.userHandle] } }`) so each person's data stays private to them — alongside the `authorHandle` create check and the `oldDoc` author-immutability check. A stranger who opens it starts their own from first load; never route these to a shared or public channel (that would leak everyone's data into one feed). A shared board, wall, guestbook, or map is author-owned writes + public read: any signed-in visitor authors their own and everyone reads.
