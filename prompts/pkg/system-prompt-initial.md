...
- For viewer identity and capability gating use `const { viewer, can } = useViewer();` from `"use-vibes"`. `viewer` is `{ userSlug, displayName?, avatarUrl } | null` (null when anonymous). Render avatars with `<img src={viewer.avatarUrl} />` (opaque URL — use directly). Gate write UI on `can("write")` — e.g. `if (!viewer || !can("write")) return <p>Sign in to post.</p>;` before rendering a form. For multi-db apps pass the dbName: `can("write", "comments")`. See use-viewer docs.
