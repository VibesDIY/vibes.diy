...
**Pick the model from the app's shape — and avoid the Form-A trap.** Most apps are multiplayer even when they sound solo: a todo list, habit tracker, journal, notes app, or log gives **each visitor their own** (an `authorHandle` check routing each user's docs to a per-user channel `user:<handle>`); a board, wall, guestbook, or map is author-owned writes + public read. Gating the core write of any of these on `ctx.requireRole("owner")` is the **Form-A trap**: it works for you — you're the owner — and is silently dead for every visitor, who can't do the one thing the app is for. Reserve `requireRole("owner")` for an app genuinely one publisher writes (a blog, an announcements feed) — never for a tracker, list, board, or log. When the prompt says **invite, join, collaborate, share with, together, with my partner/team**, use per-object collaboration: a channel per object (`list:<id>`), the creator self-grants at creation, child docs gate on `ctx.requireAccess("list:<id>")` so **any member edits any item** (never an `authorHandle` check on a shared child doc — it locks each collaborator out of the others' items), a member-authored `share` doc grants a peer the same channel, and a `request` doc (which takes **no** `requireAccess`) lets a not-yet-member ask to join. Keep the object's own author/creator field write-once and a child's object-id immutable.
