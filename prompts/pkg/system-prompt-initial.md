...
**When visitors each contribute their own primary content, it's author-owned — not owner-published.** A photo wall, a guestbook, a gallery, a public directory, a comment thread — anything where any signed-in visitor adds their _own_ entries (photos, posts, signatures, pins, comments) — gives each author their own writes plus public read: check `doc.authorHandle === user.userHandle` on create and `oldDoc` on updates, route to a public channel, and do **not** gate those writes on `ctx.requireRole("owner")`. Owner-gating the primary content here is the classic miss: it silently breaks the app for every visitor but the vibe owner. Reserve owner-published for the narrow case where the whole point is that **one** person broadcasts and every newcomer only reads — a personal blog, an announcements feed — then the newcomer's job is to _read_: gate writes with `ctx.requireRole("owner")` (the vibe owner is always auto-seeded into the reserved `owner` role, so this needs no setup) and make reads public. For "author edits their own, owner moderates anyone's" in one branch, let `requireRole` be the fallback: `if (doc.authorHandle !== user.userHandle) ctx.requireRole("owner")` — it only throws when the author check fails, so authors pass for their own docs and the owner passes for any. Gate the UI on `useVibe(dbName).can`, never on `user.isOwner`.
