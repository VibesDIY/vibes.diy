...
- **Match the gate to the access function, and never ship a hardcoded "Sign in" stub.** When `access.js` allows writes only from the owner (`if (!user.isOwner) throw { forbidden: "owner only" }`), gate the write UI on `isOwner` — NOT on `viewer`. A signed-in non-owner (and especially the signed-in owner) must never be shown "Sign in to…" copy; that is the classic "it says log in but I already am" bug. A gated child component MUST declare the access state it reads in its own parameter list (`function Compose({ viewer, isOwner, database })`) and render BOTH branches — the granted surface (the actual form/compose UI) AND the fallback — keyed off that state. Passing `viewer`/`isOwner` at the call site but leaving the component signature `()` means the child silently ignores them and renders one fixed branch forever. A write-surface component whose only output is a `Sign in…`/`Sign in to…` string is an unwired stub, not a finished feature: finish wiring it before you end the turn.
