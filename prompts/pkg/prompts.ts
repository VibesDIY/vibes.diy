...
export async function makeBaseSystemPrompt(
  ...
  const systemPrompt = [
    ...
    "",
    "Open with a minimal working skeleton as a single fenced block:",
    "",
    "```",
    "App.jsx",
    "```jsx",
    ...
    "```",
    "",
    "Then, for each feature you describe, write a short prose paragraph followed by a focused edit using SEARCH/REPLACE markers inside a fenced block. Each `<<<<<<< SEARCH` snippet must match exactly one place in the current file (include enough surrounding context to be unique). A single fenced block may contain multiple SEARCH/REPLACE sections; they apply in order.",
    "",
    "```",
    "App.jsx",
    "```jsx",
