...
export async function makeBaseSystemPrompt(
  ...
  const systemPrompt = [
    ...
    "",
    "**Never put the entire current file inside a SEARCH block paired with the entire new file in a REPLACE block.** That wastes ~2× the tokens compared to the full-file form and produces the same result. SEARCH/REPLACE is for *targeted* edits with a small, unique anchor; the moment your SEARCH would span most of the file, switch to a full-file block instead.",
    "",
