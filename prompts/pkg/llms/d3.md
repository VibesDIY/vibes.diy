...
All D3 visualizations should follow this pattern with three clear parts:

1. **Presentation Component** - Pure UI with no D3 logic, receives props and refs
2. **Visualization Functions** - D3 rendering logic separate from React
3. **Container Component** - Data management, state, and effects (the default export)

This pattern keeps D3 DOM manipulation isolated from React's rendering cycle.

### 1. Data Visualization Studio (Full Example)
...
function DataVisualizationUI({
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] p-4 font-mono">
      {/* Header */}
      <div className="mb-8">
        ...
        <p className="text-[#64748b] text-lg">
          Create and explore interactive D3.js visualizations
        </p>
      </div>

      {/* Chart Type Selector */}
      <div className="bg-[#ffffff] border-4 border-[#0f172a] mb-6 shadow-[8px_8px_0px_0px_#0f172a]">
        <div className="p-6">
          <h2 className="text-xl font-bold text-[#0f172a] mb-4">Chart Type</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {chartTypes.map((type) => (
              <button
                key={type.id}
                onClick={() => setSelectedChart(type.id)}
                className={`p-4 border-4 font-bold text-lg transition-all duration-200 min-h-[80px] ${
                  selectedChart === type.id
                    ? "bg-[#64748b] text-[#ffffff] border-[#0f172a] translate-x-[-2px] translate-y-[-2px] shadow-[6px_6px_0px_0px_#0f172a]"
                    : "bg-[#ffffff] text-[#0f172a] border-[#0f172a] shadow-[4px_4px_0px_0px_#0f172a] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#0f172a]"
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#ffffff] border-4 border-[#0f172a] shadow-[8px_8px_0px_0px_#0f172a]">
          <div className="p-6">
            <h2 className="text-xl font-bold text-[#0f172a] mb-4">
              {chartTypes.find((t) => t.id === selectedChart)?.name} Demo
            </h2>
            <div className="border-2 border-[#cbd5e1] bg-[#f1f5f9]">
        ...
        <div className="space-y-6">
          {/* Save Dataset */}
          <div className="bg-[#ffffff] border-4 border-[#0f172a] shadow-[8px_8px_0px_0px_#0f172a]">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <input
                  ...
                  className="w-full p-3 border-4 border-[#0f172a] bg-[#f1f5f9] text-[#0f172a] font-bold focus:bg-[#ffffff] transition-colors duration-200"
                />
                <button
                  onClick={onSaveDataset}
                  disabled={!datasetName.trim()}
                  className={`w-full p-3 border-4 font-bold text-lg transition-all duration-200 min-h-[48px] ${
                    datasetName.trim()
                      ? "bg-[#64748b] text-[#ffffff] border-[#0f172a] shadow-[4px_4px_0px_0px_#0f172a] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#0f172a] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_#0f172a]"
                      : "bg-[#cbd5e1] text-[#64748b] border-[#94a3b8] cursor-not-allowed"
                  }`}
                >
                  💾 Save Dataset
                </button>
                ...
                {datasets.length === 0 ? (
                  ...
                  datasets.map((dataset) => (
                    <div
                      ...
                      className="p-3 border-2 border-[#cbd5e1] bg-[#f1f5f9] hover:bg-[#cbd5e1] cursor-pointer transition-colors duration-200 min-h-[48px]"
                    >
                      ...
                      <div className="text-sm text-[#64748b]">
                        {
                          chartTypes.find((t) => t.id === dataset.chartType)
                            ?.name
                        }{" "}
...
// For simplicity, these helpers keep the example self-contained.
// In a larger app, `createVisualization` might separate one-time SVG setup
// from per-chart drawing work for better performance.
  ...
  switch (chartType) {
    ...
    case "line":
      return d3.range(20).map((i) => ({
        x: i,
        y: Math.sin(i * 0.5) * 30 + 50 + Math.random() * 20,
      }));
    case "network":
      const nodes = d3
        .range(15)
        .map((i) => ({ id: i, group: Math.floor(i / 5) }));
...
const createVisualization = (svgRef, data, chartType) => {
  ...
  const chartRenderers = {
    ...
    network: renderNetworkGraph,
  };
  chartRenderers[chartType]?.(svg, data, width, height, margin);
};

const renderBarChart = (svg, data, width, height, margin) => {
  const x = d3
    .scaleBand()
    .domain(data.map((d) => d.name))
...
const renderScatterPlot = (svg, data, width, height, margin) => {
  const x = d3
    .scaleLinear()
...
const renderLineChart = (svg, data, width, height, margin) => {
  const x = d3
    .scaleLinear()
    .domain(d3.extent(data, (d) => d.x))
    .range([margin.left, width - margin.right]);
...
const renderNetworkGraph = (svg, data, width, height) => {
  ...
  simulation.on("tick", () => {
    ...
    nodeShadow.attr("cx", (d) => d.x + 4).attr("cy", (d) => d.y + 4);
    node.attr("cx", (d) => d.x).attr("cy", (d) => d.y);
  });
};

// Container Component with data management
export default function App() {
  const { database, useLiveQuery } = useFireproof("viz-studio");
  const [selectedChart, setSelectedChart] = useState("bar");
  const [datasetName, setDatasetName] = useState("");
  const [chartData, setChartData] = useState(() => generateSampleData("bar"));
  const svgRef = useRef();

  const { docs: datasets } = useLiveQuery("type", { key: "dataset" });

  const handleSelectChart = (chartId) => {
    setSelectedChart(chartId);
    setChartData(generateSampleData(chartId));
  };
  ...
  const handleLoadDataset = (dataset) => {
    setSelectedChart(dataset.chartType);
    setChartData(dataset.data);
  };

  useEffect(() => {
    createVisualization(svgRef, chartData, selectedChart);
...
// Presentation Component
function BarChartUI({ svgRef }) {
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Simple Bar Chart</h2>
      <svg ref={svgRef} className="w-full border border-gray-300"></svg>
    </div>
  );
}

// Visualization logic
function createBarChart(svgRef, data) {
  const svg = d3.select(svgRef.current);
  svg.selectAll("*").remove();
...
export default function App() {
  ...
  return <BarChartUI svgRef={svgRef} />;
}
```
