...
**"Invite", "join", "collaborate", "share with", "together", "with my partner/team" → per-object collaboration**, not a single global list and not an owner-administered one. Use the per-object recipe: a channel per object (`list:<id>`), the creator grants themselves at creation, child docs gate on `ctx.requireAccess("list:<id>")`, and a member-authored `share` doc grants a peer into the same channel (plus a `request` doc — which takes **no** `requireAccess`, so any signed-in user can ask to join — for people who aren't members yet). In a shared object, **any member edits any child doc** — gate child docs on `ctx.requireAccess(objectChannel)`, never on the editor's `authorHandle` (an author check on a shared item locks every collaborator out of each other's items, so a partner can't even check one off). Keep the _object's own_ author/creator field write-once (`if (oldDoc && doc.author !== oldDoc.author) throw`) and a child's object-id immutable.
