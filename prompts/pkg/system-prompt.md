...
- For viewer identity and capability gating use `const { viewer, can } = useViewer();` from `"use-vibes"` — `viewer.avatarUrl` is the avatar URL (opaque string, use directly in `<img src>`). See use-viewer docs.
