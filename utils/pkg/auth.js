...
export async function extendToken(currentToken, connectApiUrl, fetchImpl = fetch) {
    try {
        ...
        if (!res.ok)
            throw new Error("Network error during token extension");
...
export function hasTenantRole(payload, tenantId, role) {
    return payload.tenants.some((tenant) => tenant.id === tenantId && tenant.role === role);
}
export function hasLedgerAccess(payload, ledgerId, right = "read") {
    return payload.ledgers.some((ledger) => ledger.id === ledgerId &&
