...
import type { UseImageGenOptions, UseImageGenResult } from "@vibes.diy/use-vibes-types";
import { imageGen } from "@vibes.diy/vibe-runtime";
...
export function useImageGen({
  ...
  const [document, setDocument] = useState<UseImageGenResult["document"]>(null);
  ...
  useEffect(() => {
    ...
    db.get(_id)
      .then((doc: unknown) => {
        setDocument(doc as UseImageGenResult["document"]);
  ...
  useEffect(() => {
    ...
    if (!imageGen) {
      setError(new Error("imageGen not available — vibe-runtime not initialized"));
      return;
    }
    ...
    imageGen(prompt)
      .then(async (urls: string[]) => {
        console.log("[ImgGen] Received image URL from service:", urls[0], "prompt:", prompt);
        ...
        setDocument(savedDoc as UseImageGenResult["document"]);
        setProgress(100);
        setLoading(false);
      })
      .catch((err: unknown) => {
        console.error("[ImgGen] Image generation failed:", err);
