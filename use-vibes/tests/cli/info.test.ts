...
import { runCli, captureOutput, withTempDir } from "./test-helpers.js";
...
describe("info", () => {
  it("shows config when vibes.json present", async () => {
    await withTempDir(async (dir) => {
      ...
      expect(captured.stdout()).toContain("test-app");
      expect(captured.stdout()).toContain("vibes.json");
    });
  });
  ...
  it("bare target shows requires login", async () => {
    await withTempDir(async (dir) => {
      ...
      expect(result.isOk()).toBe(true);
      expect(captured.stdout()).toContain("requires login");
    });
  });
  ...
  it("fully qualified target resolves", async () => {
    await withTempDir(async (dir) => {
      ...
      expect(result.isOk()).toBe(true);
      expect(captured.stdout()).toContain("alice/soup/team");
    });
  });
