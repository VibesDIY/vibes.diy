...
import { ImgVibesDisplayPlaceholder } from "@vibes.diy/base";

describe("ImgVibesDisplayPlaceholder Component", () => {
  beforeEach(() => {
  ...
  describe("Base Rendering", () => {
    it("renders waiting message when no prompt", () => {
      render(<ImgVibesDisplayPlaceholder prompt={undefined} progress={0} error={undefined} />);
      expect(screen.getByText("Waiting for prompt")).toBeInTheDocument();
    });

    it("renders prompt text when prompt is provided", () => {
      render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={0} error={undefined} />);
  ...
  describe("Error State", () => {
    it("displays error message when error is provided", () => {
      render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={0} error={new Error("Test error message")} />);
    ...
    it("handles and formats JSON error messages properly", () => {
      ...
      render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={0} error={jsonError} />);
    ...
    it("handles moderation blocked errors with special formatting", () => {
      ...
      render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={0} error={moderationError} />);
  ...
  describe("Generating State", () => {
    ...
    it("shows prompt text during generation", () => {
      render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={50} error={undefined} />);
      const matches = screen.getAllByText("Test prompt");
      expect(matches.length).toBeGreaterThan(0);
    });

    it("renders progress bar when prompt is provided", () => {
      const { container } = render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={75} error={undefined} />);
    ...
    it("enforces minimum 5% progress", () => {
      const { container } = render(<ImgVibesDisplayPlaceholder prompt="Test prompt" progress={2} error={undefined} />);
      act(() => {
        vi.advanceTimersByTime(25);
      });
      const progressBar = container.querySelector('[aria-hidden="true"]');
    ...
    it("does not render progress bar without a prompt", () => {
      const { container } = render(<ImgVibesDisplayPlaceholder prompt={undefined} progress={50} error={undefined} />);
      const progressBar = container.querySelector('[aria-hidden="true"]');
