...
describe("ImgVibes Document ID Tracking", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should track document ID when starting with just a prompt", async () => {
    // Render ImgVibes with just a prompt
    const { container } = render(<ImgVibes prompt="Test prompt" data-testid="img-vibes" />);
    ...
    const imgVibesElement =
      container.querySelector('[data-testid="img-vibes"]') ||
      container.querySelector(".img-vibes-upload-waiting") ||
      container.firstElementChild;

    expect(imgVibesElement).toBeInTheDocument();
  });
});
