...
vi.mock("call-ai", async () => {
  ...
  return {
    ...(actual as object),
    imgVibes: mockData.mockImageGen,
  };
});
...
describe("ImgVibes Component", () => {
  ...
  it("should render a placeholder while loading", () => {
    ...
    render(<ImgVibes prompt="test image" />);
  ...
  it("should attempt image generation with correct parameters", async () => {
    ...
    // Render the ImgVibes component with our test configuration
    render(<ImgVibes prompt="beautiful landscape" options={customOptions} />);

    // Wait for the setTimeout in useImgVibes (it uses 10ms)
    await act(async () => {
  ...
  it("should handle errors gracefully", async () => {
    // Test error handling by providing a custom useImgVibes mock that returns an error state
    const mockUseImgVibesWithError = vi.fn().mockReturnValue({
      imageData: null,
      loading: false,
      progress: 0,
    ...
    // Render with custom useImgVibes hook that simulates error state
    const { container } = render(<ImgVibes prompt="error prompt" useImgVibes={mockUseImgVibesWithError} />);
    ...
    expect(mockUseImgVibesWithError).toHaveBeenCalledWith(
  ...
  it("should accept custom props", async () => {
    ...
    const component = <ImgVibes {...props} />;
    expect(component.props).toEqual(props);
  });

  it('should show "Waiting for prompt" when prompt is falsy', async () => {
    // Create a custom useImgVibes mock that simulates no prompt/no document state
    const mockUseImgVibesEmpty = vi.fn().mockReturnValue({
      imageData: null,
      loading: false,
      progress: 0,
      error: null,
    ...
    // Render with custom useImgVibes hook and empty prompt
    const { container } = render(<ImgVibes prompt="" useImgVibes={mockUseImgVibesEmpty} />);
    ...
    expect(mockUseImgVibesEmpty).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: "",
    ...
    // Verify imgVibes is not called when prompt is empty
  ...
  it("should not display progress when no request is being made", () => {
    ...
    // Our mock useImgVibes sets loading=false for empty prompt
    // Provide _id to prevent validation error
    const { container } = render(<ImgVibes prompt="" _id="test-image-id" />);
  ...
  it("should show generating state when _id is provided and document contains prompt", async () => {
    // This test verifies that the ImgVibes component can extract a prompt from a document
    // and use it for generation when only an _id prop is provided (no prompt prop)

    // Create a custom useImgVibes mock that simulates finding a document with a prompt
    const mockUseImgVibesWithDocument = vi.fn().mockReturnValue({
      imageData: null,
      loading: true,
      progress: 25,
      error: null,
    ...
    // Render with custom useImgVibes hook and only _id prop, no prompt prop
    const { container } = render(<ImgVibes _id="test-doc-with-prompt" useImgVibes={mockUseImgVibesWithDocument} />);
    ...
    expect(mockUseImgVibesWithDocument).toHaveBeenCalledWith(
      expect.objectContaining({
        _id: "test-doc-with-prompt",
      })
    );
