...
const MountVibesAppExample = ({ setCurrentExample }: MountVibesAppExampleProps) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // todo this is the wrong arguments type
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "800px", margin: "0 auto" }}>
      ...
      <p style={{ marginBottom: "2rem", color: "#666", fontSize: "1.1rem" }}>
        Testing the unified mount function that wraps existing content with auth wall → vibes switch flow. This uses the local
        bundler for faster iteration.
      </p>
      ...
      <div>
        ...
        >
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            {timeStamp && <span style={{ color: "#666", fontSize: "14px" }}>Last clicked: {timeStamp}</span>}
          </div>

          {/* Input test */}
          <div style={{ marginBottom: "20px" }}>
            <label style={{ display: "block", marginBottom: "5px", color: "#333" }}>Text Input (should maintain state):</label>
            <input
              type="text"
              value={inputValue}
            ...
            />
            <p style={{ fontSize: "12px", color: "#666", margin: "5px 0 0 0" }}>Current value: "{inputValue}"</p>
          </div>
