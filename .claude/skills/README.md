...
[`autoresearch/`](autoresearch/SKILL.md) — autonomous goal-directed iteration: set a GOAL, run a **modify → verify → keep/discard → repeat** loop against a measurable metric, auto-keeping wins and reverting regressions. Bare `/autoresearch` routes to 13 subcommands (`plan`, `debug`, `fix`, `security`, `ship`, `scenario`, `predict`, `learn`, `reason`, `probe`, `improve`, `evals`, `regression`) under [`.claude/commands/autoresearch/`](../commands/autoresearch/). Vendored as a flat project skill + commands (+ its `orchestrate.sh` / `score-regression.sh` helper scripts) so it works in **cloud sessions** (no global plugin install). Of the upstream's 9 hooks, only the **two fail-open safety guards** — `dangerous-cmd-block` (force-push/destructive bash) and `privacy-block` (credential-file reads) — are vendored, wired via [`.claude/settings.json`](../settings.json); note these fire on **every** session in the repo, not just `/autoresearch` runs. The other 7 (loop-context injectors, notifications, the opinionated `simplify-gate`) are intentionally excluded. Provenance, hook scope, and what was excluded live in [`autoresearch-vendor/`](autoresearch-vendor/README.md).

### Vendored OpenRouter skills ([OpenRouterTeam/skills](https://github.com/OpenRouterTeam/skills), MIT)

16 skills for the [OpenRouter](https://openrouter.ai) API, vendored as flat project skills (so they work in **cloud sessions**) via `gh skill install OpenRouterTeam/skills --all --agent claude-code --scope project`. Added to support the [`eval/codegen-matrix`](../../eval/codegen-matrix/README.md) cross-model work — calling 300+ models through one API, capturing per-generation cost/tokens, and building the agentic-loop eval mode.

- **Core for the eval:** [`openrouter-typescript-sdk/`](openrouter-typescript-sdk/SKILL.md) (the `callModel` pattern), [`openrouter-agent-migration/`](openrouter-agent-migration/SKILL.md) (`@openrouter/agent`: `tool()`, stop conditions, streaming — the agentic loop), [`openrouter-generations/`](openrouter-generations/SKILL.md) (per-generation cost/latency/tokens), [`openrouter-models/`](openrouter-models/SKILL.md) + [`openrouter-benchmarks/`](openrouter-benchmarks/SKILL.md) (pricing/discovery/rankings).
- **Also installed:** `open-responses`, `create-agent-tui` + `create-headless-agent` (TypeScript agent scaffolders, incl. their `sample/` apps), `openrouter-images`/`video`/`tts`/`stt`, `openrouter-oauth`, and the `openrouter-analytics{,-schema,-query}` trio.
- **Runtime env:** the runnable `scripts/*.ts` need `OPENROUTER_API_KEY` (and optional `OPENROUTER_BASE_URL`) — get a key at [openrouter.ai/keys](https://openrouter.ai/keys). Scanned at vendor time: the scripts call only `openrouter.ai`; the `child_process`/`spawn` uses are confined to the `create-*` sample agents' own file/shell tools.
- **Upgrade:** re-run the `gh skill install … --force` command above to pull upstream changes (no auto-update at project scope).
