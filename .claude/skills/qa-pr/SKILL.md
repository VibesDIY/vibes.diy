...
What's automatic, and the one thing that isn't:
...
- **Authenticated sign-in** — the cloud Chrome starts cold with **no Google session**, so the Step 4.1 one-click "Continue as &lt;you&gt;" OAuth has nothing to continue from. Seeding a Google/Vibes session into a headless cloud profile is **not yet automated**. Until it is, run the authenticated full spine (Steps 4–7) where a session can be established — either a local workstation with a seeded profile (below), or by first establishing a Vibes session in the cloud profile. Don't fake it: if you can't sign in, finish the unauthenticated checks and say in the triage that the authenticated spine was `path_not_tested` with the reason.
...
- **(Authenticated spine only)** Verify a usable Google session before Steps 4–7: `new_page` to <https://accounts.google.com>, take a snapshot, and confirm a signed-in identity appears. On a **workstation** this comes from the seeded profile ([`references/local-workstation-setup.md`](references/local-workstation-setup.md)). In a **cloud session** there is no seeded Google session yet (see _Browser environment_ → cloud) — if you can't establish one, stop before Step 4, run Steps 1–3 only, and mark the authenticated spine `path_not_tested` in the triage rather than faking a sign-in.
