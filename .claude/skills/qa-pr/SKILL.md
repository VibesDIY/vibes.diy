...
This skill drives the browser **only** through the `mcp__chrome-devtools__*` tools, and the **Claude Code cloud / web session is the default place to run it** — prefer it over a local workstation.

### Cloud session (default)

In a cloud session the browser is provisioned automatically: a SessionStart hook runs [`scripts/setup-cloud-browser.sh`](../../../scripts/setup-cloud-browser.sh), which makes chrome-devtools MCP's headful Chrome work through the agent egress proxy. You install nothing. If `navigate_page` fails with `ERR_CONNECTION_CLOSED` or a "can't find Chrome" error, the hook didn't run — run `bash scripts/setup-cloud-browser.sh` once and retry. Full background: [`agents/cloud-browser-setup.md`](../../../agents/cloud-browser-setup.md).
...
Running on a developer Mac/Linux box drives a real, installed Chrome with a persistent profile — currently the path for the authenticated spine, until cloud auth seeding lands. It has its own one-time setup (real Chrome, GitHub CLI, a seeded Google Workspace session in the chrome-devtools profile): [`references/local-workstation-setup.md`](references/local-workstation-setup.md).

Either way, the **chrome-devtools MCP server** itself is provisioned by the repo's root [`.mcp.json`](../../../.mcp.json) (`npx -y chrome-devtools-mcp@latest`); approve it on repo-trust (or `/mcp` → enable `chrome-devtools`) so the `mcp__chrome-devtools__*` tools appear. **`gh` authenticated**, **Node**, **git email set**, and **a Vibes account** are required in both environments — Step 1 preflight checks them.
...
- **Browser actually launches** — after confirming the tools exist, do a throwaway `navigate_page` to `https://example.com` and confirm it loads. In a **cloud session** this is automatic (SessionStart hook); if it fails with `ERR_CONNECTION_CLOSED` or a missing-Chrome error, run `bash scripts/setup-cloud-browser.sh` and retry (see [`agents/cloud-browser-setup.md`](../../../agents/cloud-browser-setup.md)). On a **local workstation**, ensure real Chrome is installed per [`references/local-workstation-setup.md`](references/local-workstation-setup.md) (`open -Ra "Google Chrome"` on macOS).
...
- Read [`references/chrome-mcp-rules.md`](references/chrome-mcp-rules.md) and confirm via `evaluate_script` of `document.cookie` on the preview URL that the browser profile is clean of Vibes session state (no `__session` cookie, no signed-in `__client_uat` value, no `vibes.diy`-scoped cookies). A cloud session's Chrome is cold by construction; on a workstation, if the profile is dirty, abort and instruct the operator to either complete Step 7's sign-out cleanup from the previous run, or wipe the profile per [`references/local-workstation-setup.md`](references/local-workstation-setup.md) and re-sign-in to Google.
- **(Authenticated spine only)** Confirm you can sign in before Steps 4–7. In a **cloud session**, check `CLERK_SECRET_KEY` is set for the target instance and use the Clerk sign-in-token flow (see _Browser environment_ → cloud); confirm `window.Clerk.user?.id` is populated after consuming the token. On a **workstation**, verify the seeded Google session (`new_page` to <https://accounts.google.com>, snapshot, confirm a signed-in identity — [`references/local-workstation-setup.md`](references/local-workstation-setup.md)). If neither is available, stop before Step 4, run Steps 1–3 only, and mark the authenticated spine `path_not_tested` rather than faking a sign-in.
