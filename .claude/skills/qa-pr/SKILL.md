...
- **Authenticated sign-in (cloud) — via a Clerk sign-in token.** The cloud Chrome starts cold with no Google session, so interactive OAuth is a non-starter. Instead, log in through Clerk's own [sign-in-token / ticket flow](../../../docs/specs/2026-06-28-clerk-signin-token-qa-login.md) — it yields a **real Clerk session** (identical to a normal login), so the rest of the spine runs unchanged.
  1. **Mint** (needs `CLERK_SECRET_KEY` for the **target instance** in the harness env — prod for `vibes.diy`, dev/preview for `*.workers.dev` previews):
     ```bash
     TOKEN=$(CLERK_SECRET_KEY=$CLERK_SECRET_KEY node .claude/skills/qa-pr/scripts/clerk-signin-token.mjs)
     ```
     (resolves your Clerk userId by `git config user.email`; pass `--email`/`--user-id` to override.)
  2. **Consume** on the preview origin (where `window.Clerk` is live) via `evaluate_script`, interpolating `$TOKEN` into the snippet: `await window.Clerk.client.signIn.create({ strategy: "ticket", ticket })` then `await window.Clerk.setActive({ session: result.createdSessionId })`. Verify `window.Clerk.user?.id` is set, then proceed from Step 4.
