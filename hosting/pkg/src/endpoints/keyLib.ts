...
/**
 * Increases a key's credit limit by a specified dollar amount
 *
 * This function first retrieves the current key details using getKey(),
 * then adds the specified amount to the current limit, and finally
 * updates the key with the new limit using updateKey().
 *
 * @param params - Parameters for increasing key limit
 * @param params.hash - The hash of the key to increase the limit for
 * @param params.amount - The dollar amount to increase the limit by
 * @param params.provisioningKey - The OpenRouter provisioning API key
 * @returns A Promise resolving to a KeyResult with success status and key data
 */
export async function increaseKeyLimitBy(params: {
  hash: string;
  amount: number;
  provisioningKey: string;
}): Promise<KeyResult> {
  const { hash, amount, provisioningKey } = params;
  ...
  try {
    ...
    const currentUsage = getResult.key.usage || 0; // Default to 0 if usage is undefined
    const availableBalance = currentLimit - currentUsage;
    ...
    if (availableBalance >= 1) {
      ...
      return {
        success: true,
        key: getResult.key, // Use the existing key data
      };
    }
