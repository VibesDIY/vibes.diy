...
- **Claim an issue before working it** — when told to work on an issue by number, the first move (before investigating or coding) is to self-assign it to `jchris` so the same work doesn't get started twice. Policy: [pr-lifecycle.md § Claim an issue before working it](agents/pr-lifecycle.md#claim-an-issue-before-working-it).
- Never push to main
