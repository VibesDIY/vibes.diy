...
import { buildRequestBody, SchemaInput } from "./build-request.js";
import {
...
const app = command({
  ...
  handler: async ({
    ...
    if (src) {
      ...
      const styleOverride = apiStyleArg === "openai" || apiStyleArg === "anthropic" ? apiStyleArg : undefined;
      const requestBody = buildRequestBody({ model, prompt, schema: schemaObj, apiStyle: styleOverride, url });
      ...
      const isAnthropic = requestBody.tools !== undefined;
