...
let openaiSseLines: string[] = [];

beforeAll(async () => {
  openaiSseLines = await loadFixtureLines("openai-json-schema.llm.txt");
});
...
describe("openai SSE format through v2 pipeline", () => {
  it("full pipeline produces valid sandwich JSON from OpenAI SSE", async () => {
    ...
    const res = await stream2array(
      array2stream(openaiSseLines.map((i) => i + "\n"))
  ...
  it("delta-stream produces correct content from OpenAI SSE", async () => {
    let id = 1;
    const streamId = `test-${id++}`;
    const res = await stream2array(
      array2stream(openaiSseLines.map((i) => i + "\n"))
  ...
  it("sse-stream handles OpenAI direct format (no provider/native_finish_reason)", async () => {
    let id = 1;
    const streamId = `test-${id++}`;
    const res = await stream2array(
      array2stream(openaiSseLines.map((i) => i + "\n"))
