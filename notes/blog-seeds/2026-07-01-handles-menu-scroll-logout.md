...
viewport with nowhere to scroll — and because the menu was hard-coded
`placement="down"`, it grew *toward* the screen edge that had the least room.

Three moves, all small: wrap only the `handles.map` in a `maxHeight: 40vh;
overflowY: auto` scroll region (so the list scrolls but the "New handle" and new
"Log out" rows stay pinned and reachable no matter how long the roster is), flip
the card's usage to `placement="up"` (the prop already existed — there's just
more room above a card than below it), and add an optional `onLogout` row at the
bottom wired to the same `clerk.signOut()` the SessionSidebar and Settings page
use.
