...
- **Tech**: `use-vibes login` — device-code auth flow, stores credentials locally. `use-vibes whoami` — prints device identity + linked handles (fetched from API). Required before `live` or `publish`
