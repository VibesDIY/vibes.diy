...
> **Status:** the **encapsulation boundary ("far shore")** has landed on this branch — every identity-relevant `@fireproof/*` import in source routes through `@vibes.diy/identity` (`.` browser-safe / `./server` worker-safe / `./node` Node-CLI), the three duplicated signers are DRY'd, and the Clerk client is re-homed — while the package stays fireproof-backed internally. This subsumed Plan 2 and the import-routing portion of Plan 3 (the lift-verbatim _internals_ swap is the remaining, deferred "deep water"). Deferred tail tracked as issues: [#2468](https://github.com/VibesDIY/vibes.diy/issues/2468) (RuntimeContext narrowing / Plan 5), [#2469](https://github.com/VibesDIY/vibes.diy/issues/2469) (browser-graph hardening — the durable replacement for the `find-up` `optimizeDeps.exclude`), [#2470](https://github.com/VibesDIY/vibes.diy/issues/2470) (CLI-framework extraction off `@fireproof/core-cli` / Plan 7).
