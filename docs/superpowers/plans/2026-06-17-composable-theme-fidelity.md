...
- [ ] **Step 1 — failing tests.** In a new `describe("deriveCanonical backfills every core slot")` in `themes.test.ts`, assert that for a yaml with only `background` + `text-primary`, the composed `:root` contains `--surface`, `--secondary`, `--border`, `--text-secondary`, `--primary`, `--accent` (non-empty). Add a test that the emitted `classNames` example parses: `expect(() => new Function(exampleBlockJs)).not.toThrow()` (extract the ```js block). Run `cd vibes.diy/tests && pnpm vitest run ../../prompts/tests/themes.test.ts` → expect FAIL.
- [ ] **Step 2 — implement backfill** in `deriveCanonical()`: add `surface←background`, `secondary←accent??primary`, `border←text-primary??neutral`, `text-secondary←color-mix(in srgb,text-primary 60%,background)` (guard when missing), and last-resort `background`/`text-primary` neutrals. Keep existing primary↔accent + state fills. Order matters — fill `primary/accent` first, then `secondary`, then surface/border/text-secondary.
...
- [ ] **Step 2 — run** `cd vibes.diy/tests && pnpm vitest run ../../prompts/tests/theme-completeness.test.ts` → all 44 PASS (this is the proof tasks 1–2 worked). Fix data/derivation until green.
