...
- `rollCodegenShard()` (exposed on the API interface): increment `currentShardIndex`, persist it to `sessionStorage` (key `vibes-codegen-shard-idx:<userId>`), drop `currentConnection`, and `getReadyConnection()` again on the new shard. Returns `true` if it rolled, `false` once `MAX_ROLL_INDEX` is hit or the connection isn't rollable — so callers stop retrying.
- **Recovery is chat-session-driven, not a transparent `request()` retry (Charlie review).** A prompt's section stream is bound to the connection at `LLMChat` open time, and the server requires `openChat` to have run on the _same_ connection (`conn.chatIds.get(chatId)`, else `Chat context not found`). So a transparent re-send on the rolled socket would stream into a dead listener AND hit a context-less socket. Instead, `useChatSession` owns recovery: on `shard-overloaded` it calls `rollCodegenShard()`; if that rolled, it stashes the prompt, dispatches `rollReconnect` (forces `connection: "reconnecting"` even while idle — a bare `streamDisconnected` is a no-op pre-block), the reconnect loop re-runs `openChat` on the rolled shard, and `handleReconnectAttempt` re-fires the stashed prompt **once** (later attempts replay, not re-send). If `rollCodegenShard()` returns `false` (bound hit), the error is surfaced normally and the submit guard released.
