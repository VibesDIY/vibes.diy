...
The per-handle avatar write goes through a handle-scoped ensure (modeled on `ensureAppSettings`; see Q4) carrying the target `handle`, gated by handle ownership. The host-side preview/confirm gate from #2418 stays; it becomes keyed on `(user, handle)` and the modal copy can name the handle being changed. The sandbox `updateAvatarCid` request already carries `ownerHandle` (via `vibeApp`), and Settings knows the selected handle, so the plumbing exists.
...
4. **Write surface + displayName.** New `ensureHandleSettings` handler, or extend the `ensureAppSettings`-style machinery to a handle key? And does per-handle **`displayName`** move into `handleSettings` too (deprecating `userSettings.profile.displayName`), or stay per-user for now? Avatar-only is a smaller first step.
